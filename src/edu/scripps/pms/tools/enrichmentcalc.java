...
public class enrichmentcalc {
	...
	public double[][] Go(String[] args, String[] aanames, int[][] comp) throws Exception {
                int             dirIterator;
		int 		i;
		...
		numsteps = (int)((enrich_max - enrich_min) / enrich_delta);
                resultarray = new double[numsteps+10][2];
		...
		while (spectrumList.hasNext() && !found) {
			...
			if (list.getLoscan() == scan) {
				...
				composition = calcComposition(sequence, aanames, comp);
				...
				while(enrichment >= enrich_min) {
					...
					//for (i=0;i<arraysize;i++) {
                        		//	System.out.println(masslist[i] + "\t" + exp_abun[i] + "\t" + theo_abun);
                			//}
					...
					resultarray[specCount][0] = enrichment;
					resultarray[specCount][1] = maxcorr; 
	...
	public int[]  calcComposition (String sequence, String[] aanames, int[][] comp) {
	...
	private void sumElements(int[][] arr, int j){
