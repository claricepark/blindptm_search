...
public class OUTFile {
    ...
    /* This is the magical function by which a list of OUTFiles
     * becomes a list of Proteins.  OUTFile lists are one-dimensional;
     * each OUTFile points to another OUTFile, and each OUTFile points
     * to only one DTAFile.  Protein lists are two-dimensional; each
     * Protein points to another Protein, and each Protein points to a
     * list of DTAFiles.  This function should always be called on an
     * OUTFile list that has been sorted by locus.  For each group of
     * OUTFiles that have the same locus, the DTAFiles are collated
     * into a list and that list is attached to a new Protein object
     * that is added to a growing list of Proteins.  To maintain the
     * order in which the OUTFiles were sorted, the Protein objects
     * are tail added to the growing list.
     */
    ...
    /* Read the unified SEQUEST result file.  Create an OUTFile object
     * for each peptide for each protein.  
     * 
     * Format of .sqt file is as follows:
     * For each S line, there should be a series of M lines.  After
     * each M line, there is one or several L lines.  S lines indicate
     * a new spectrum (or a different charge state for a previously
     * examined spectrum).  M lines indicate matching sequences for
     * the most recent spectrum.  L lines indicate database loci
     * containing the sequence reported in the last M line.
     *
     * This function separates the file into sections starting with S
     * lines.  Each M line with a deltCN of 0.0 is multiplexed with
     * the L lines below to create OUTFile objects that are appended
     * to the MasterList.  If there's an error in one of these groups,
     * the objects for that spectrum are removed (by keeping a pointer
     * to the last MasterList until all is clear).  Once a nonzero
     * DeltCN is found, the series of created OUTFile objects for this
     * spectrum are marked with the DeltCN, and the file is advanced
     * to the next S line.
     */
    public static int ReadSQT(File CurrentDirectory,
			      String Filename,
			      OUTFile MasterList,
			      IniFile Config,
	...
	while (WholeLine != null) {
		...
		while ( (bDeltCN == 0.0f) &&
			...
			bCalcPreMass = new Float(Parser.nextToken()).floatValue();
			// Skip the DeltCN
			Parser.nextToken();
			// Store XCorr
			bXCorr = new Float(Parser.nextToken()).floatValue();
			// Skip the Sp score
			bSpScore = new Float(Parser.nextToken()).floatValue();
			...
			LocusCount = 0;
			LastReadLocus = "";
			while ( (WholeLine != null) && (WholeLine.startsWith("L\t")) ) {
			    /* When we get a locus name, we can create an
			     * OUTFile storing the information we've
			     * accumulated in the above S and M lines. */
			    Parser = new StringTokenizer(WholeLine);
			    // Skip the line label
                            Parser.nextToken();
			    // Trim this locus to 40 characters
                            ...
                            if (TempRawLocus.length() > Config.LocusLengthCutoff)
                                TempRawLocus = TempRawLocus.substring(0, Config.LocusLengthCutoff);
				...
				LastReadLocus = TempLocus;
			        ...
			        FoundMLine = true;
			        // Store the information we've gathered in the new object
			        ...
			        CurrentDTA.FileName = bFileName;
                                String[] ParsedLine = CurrentDTA.FileName.split("\\.");
			        CurrentDTA.ChargeState = new Byte(ParsedLine[3]).byteValue();
			        CurrentDTA.ScanNumber = new Integer(ParsedLine[1]).intValue();
				CurrentDTA.RootFileName = ParsedLine[0];
			        ...
			        CurrentDTA.TrypticSites = CurrentDTA.DetermineMissedTryptic(Cutoffs);
			...
			if ( (SetDeltCN != 0) && (DTAFile.JustLettersFromTrimmed(SecondSequence)).equals(DTAFile.JustLettersFromTrimmed(bSequence)) ) {
			...
			else if ( (SetDeltCN != 0) && (bSp == 1) && (bXCorrRank - SecondXCorrRank > 1) ) {
		...
		System.out.println("\tA number is formatted incorrectly in " + bFileName);
	    }
	    catch (NoSuchElementException failure) {
		System.out.println("\tA line did not have as many elements as it should in " + bFileName);
	    }
    ...
    public static int ReadMascotDAT(String Filename,
				    OUTFile MasterList,
	...
	while (PeptidesWholeLine.startsWith("q")) {
		...
		if (Parser.nextToken().equals("-1")) {
			...
			if (QueryWholeLine == null) {
			    System.out.println("Hit bottom of file while looking up query title " +
					       new Integer(QueryNum).toString());
				...
				bFileName = bFileName.substring(0, bFileName.length() - 4);
			    }
			    else
				bFileName = Filename + QueryNum;
			}
			...
			if (SummaryWholeLine == null) {
			    System.out.println("Hit bottom of file while looking up summary." +
					       TempLocus);
			    SummaryFilereader = new FileReader(DatFile);
			    SummaryIncoming = new BufferedReader(SummaryFilereader);
			    SummaryWholeLine = SummaryIncoming.readLine();
			    QueryZ = 2;
			    bQmatch = 1;
			}
			    ...
			    TempLocus = "qmatch" + new Integer(QueryNum).toString() + "=";
			    SummaryWholeLine = SummaryIncoming.readLine();
			    while ( (SummaryWholeLine != null) &&
				    (!SummaryWholeLine.startsWith(TempLocus)) ) {
				SummaryWholeLine = SummaryIncoming.readLine();
				...
				Parser2 = new StringTokenizer(SummaryWholeLine, "=");
				Parser2.nextToken();
				bQmatch = new Integer(Parser2.nextToken()).intValue();
			    }
			}
		    }
		    /*
		      System.out.println(new Integer(QueryNum).toString() + "\t" +
		      new Byte(QueryZ).toString() + "\t" +
		      new Integer(bQmatch).toString());
		    */
		    // Process this query
		    bObsPrecursorMPlusH = new Float(Parser.nextToken()).floatValue() +
		    ...
		    bModString = bModString.substring(1,bModString.length() - 1);
		    bIonsScore = new Float(Parser.nextToken()).floatValue();
		    bThreshold = new Float(10.0f *
					   (Math.log(bQmatch))/
					   (Ln10)).floatValue();
		    bDeltCN = Protein.RoundTo(new Float(
					Math.pow(10f, bIonsScore / 10.0f) /
					new Double(bQmatch).doubleValue()).floatValue(), 2);
		    /*
		    System.out.println(new Integer(QueryNum).toString() + "\t" +
				       new Integer(MatchNum).toString() + "\t" +
				       bFileName + "\t" +
				       bSequence + "\t" +
				       new Float(bIonsScore).toString() + "\t" +
				       new Float(bThreshold).toString() + "\t" +
				       new Integer(bQmatch).toString() + "\t" +
				       new Float(bDeltCN).toString());
		    */
		    /*
		      System.out.print(new Integer(QueryNum).toString() + "\t" +
		      new Integer(MatchNum).toString() + "\t" +
		      new Float(bObsPrecursorMPlusH).toString() + "\t" +
		      new Integer(bIonsMatched).toString() + "\t" +
		      bSequence + "\t" +
		      bModString + "\t" +
		      new Float(bIonsScore).toString() + "\t");
		    */
		    ...
		    // Now split this string on commas (which separate locus sections)
			...
			CurrentDTA.IonProportion = new Float(bIonsMatched).floatValue() /
			    new Float( (QueryZ > 2) ?
			      4 * (bSequence.length()-1) :
			      2 * (bSequence.length()-1) ).floatValue();
			CurrentDTA.Sequence = SequenceBuilder.toString();
		    }
		    // If the next match's Ions Score is the same, handle it next
		    PeptidesWholeLine = PeptidesIncoming.readLine();
		    Parser = new StringTokenizer(PeptidesWholeLine, ",");
		    // Skip q#_p#=
		    Parser.nextToken();
	...
	while (ORunner != null) {
	    System.out.println(ORunner.Locus + "\t" +
			       ORunner.DTA.PrecursorMass + "\t" +
			       ORunner.DTA.FileName + "\t" +
			       ORunner.DTA.Subdirectory + "\t" +
			       ORunner.DTA.TotalIntensity + "\t" +
			       ORunner.DTA.Sp + "\t" +
			       ORunner.DTA.XCorr + "\t" +
			       ORunner.DTA.DeltCN + "\t" +
			       ORunner.DTA.PepConf + "\t" +
			       ORunner.DTA.PepFP + "\t" +
			       ORunner.DTA.IonProportion + "\t" +
			       ORunner.DTA.Sequence);
	    ORunner = ORunner.Next;
	}
    }

    public static void GetVersion(File CurrentDirectory,
				  String Filename,
    ...
    /* In an effort to clean up the reading of .out files, I've copied
     * my code and hope to make this function more flexible in how it
     * reads .out files as well as more comprehensible.
     * 1) multiple loci may be listed after the first matching
     * peptide.  This is flagged by the presence of a +x before the
     * first matching sequence.  The other matching loci are the
     * first tokens on the succeeding lines.  The number of other
     * matching loci is limited to 20, although the +x may indicate a
     * larger number of possible loci for this match.
     *
     * 2) The DeltCN value in line 2 may be 0.0.  This could result
     * from an exchange of L for I or Q for K, but there have been
     * reports of identical sequences appearing in lines one and 2 as
     * well.  Suffice to say that if a 0.0 DeltCN is found in line 2,
     * the DeltCN on line 3 is sought out.  
     *
     * If only one locus maps to this sequence, an isolated OUTFile is
     * returned.  If multiple loci map to this sequence, a linked
     * list of OUTFiles is returned.
     */
    public static OUTFile ReadOUT(File CurrentDirectory,
				  String Filename,
				  boolean ThisDirOnly,
				  IniFile Config,
                                  SelectCriteria Cutoffs) throws IOException {
	...
	try {
		...
		if (Parser.hasMoreTokens()) {
		    LineBuffer = Parser.nextToken();
		    // If this is a sequence line, we're out of the header portion
			...
			if (WholeLine.length() == 0) {
			    System.out.println("\t" + Filename + " did not list any matches!");
			    return null;
			}
			Parser = new StringTokenizer(WholeLine," \t/=,(");
			LineBuffer = Parser.nextToken();
		    ...
		    if (WorkingOnHeader) {
			/* We haven't reached the "1." line yet.
			 * Note that if total intensity and
			 * precursor mass are on the same line,
			 * we'll only be able to get the first.
			 * In SEQUEST-SNP files, precursor mass is
			 * on the same line as a mass tolerance,
			 * so it's required that we kill the rest
			 * of the line. */
			while (Parser.hasMoreTokens()) {
			    /* We can't be sure which line has the
			     * total intensity and precursor mass
			     * information, so we'll read all of them,
			     * moving word by word. */
			    if (LineBuffer.equals("total")) {
				/* The token after next should be
				 * total intensity.
				 */
				LineBuffer = Parser.nextToken();
				bTotalIntensity = new Float(Parser.nextToken()).floatValue();
			...
			/* We're past the header.  This must be a
			 * numbered line. */
			...
			if (bDeltCN == 0.0f) {
			    ...
			    Parser = new StringTokenizer(WholeLine," \t/");
			    Parser.nextToken();
			    // Make a new OUTFile object to store this set of IDs
			    ...
			    Parser.nextToken();
			    CurrentDTA.Sp = new Short(Parser.nextToken()).shortValue();
			    if (IncludesIDField)
				Parser.nextToken();
			    CurrentDTA.CalcPreMass = new Float(Parser.nextToken()).floatValue();
			    CurrentDTA.GetMassOffsets(Cutoffs);
			    Parser.nextToken();
			    CurrentDTA.XCorr = new Float(Parser.nextToken()).floatValue();
			    if (CurrentDTA.XCorr == 0) {
				System.out.println("\t" + Filename + " has XCorr of 0.0!");
				return null;
			    }
			    CurrentDTA.SpScore = new Float(Parser.nextToken()).floatValue();
			    if (IncludesSfField)
                                Parser.nextToken();
				...
				OUTRunner.RawLocus = LineBuffer.substring(0, Config.LocusLengthCutoff);
			    else
				OUTRunner.RawLocus = LineBuffer;
			    if (Cutoffs.ShortForm) {
			        OUTRunner.Locus = NRUtils.getAccession(OUTRunner.RawLocus);
			    }
			    else {
				...
				// There will be more loci containing this sequence
				// Currently, LineBuffer is not on the sequence
				LineBuffer = Parser.nextToken();
			    }
			    else {
				// This is a unique ID; LineBuffer contains the sequence.
				CurrentDTA.UniqueToLocus = true;
			    }
			    // Check for SEQUEST-SNP formatted sequence
			    if (LineBuffer.charAt(0) == '(') {
				CurrentDTA.Sequence = new Character(LineBuffer.charAt(1)).toString() +
				    "." + 
				...
				System.out.println("\t" + Filename + " reports no sequence!");
				return null;
			    }
			    CurrentDTA.DetermineTryptic(Cutoffs);
			    CurrentDTA.DetermineModified();
				...
				if (LineBuffer.charAt(LineBuffer.length()-1) == '.') {
				    // This is a numbered line!  Go back through larger loop.
				    Parser = new StringTokenizer("");
				}
				else {
				    if (IncludesIDField)
					LineBuffer = Parser.nextToken();
				    /* Since loci appearing on the
                                     * line with XCorr are limited to
                                     * only 40 characters, limit this
                                     * one to that length, too.  */
                                    if (LineBuffer.length() > Config.LocusLengthCutoff)
                                        TempRawLocus = LineBuffer.substring(0, Config.LocusLengthCutoff);
                                    else
				    ...
				    if (!TempLocus.equals(LastReadLocus)) {
				        // This is an additional locus containing this sequence
					LastReadLocus = TempLocus;
				        OUTRunner.Next = new OUTFile();
				        OUTRunner = OUTRunner.Next;
					OUTRunner.Locus = TempLocus;
					OUTRunner.RawLocus = TempRawLocus;
				        // Write the invariant fields for this .out file
				        NewDTA = OUTRunner.DTA;
				        NewDTA.PrecursorMass = bPrecursorMass;
				        NewDTA.FileName = bFileName;
                                        ParsedLine = NewDTA.FileName.split("\\.");
                                        NewDTA.ChargeState = new Byte(ParsedLine[3]).byteValue();
				        NewDTA.ScanNumber = new Integer(ParsedLine[1]).intValue();
					NewDTA.RootFileName = ParsedLine[0];
				        NewDTA.Subdirectory = bSubdirectory;
				        NewDTA.TotalIntensity = bTotalIntensity;
				        // Write the sequence-specific information
				        ...
				        NewDTA.TrypticSites = NewDTA.DetermineMissedTryptic(Cutoffs);
			    ...
			    // We've hit a nonzero DeltCN and are done reading the file.
			    NoDeltCNYet = false;
    ...
    /* Dave's amazing quicksorter.  I leave it as an exercise for the
     * reader to determine how this works.
     */
