...
public class SelectCriteria {
    ...
    */
    ...
    /* If the user has commonly used options for DTASelect, he or she
     * can write them into a file called DTASelect.params and copy
     * them around rather than continually retype the same
	...
	for(i = 0; i < args.length; i++) {
		...
		/* Require that included peptides have sequences
		 * containing all of these residues somewhere before C
		 * terminus */
		...
		/* Require that included peptides have sequences
		 * excluding all of the following residues before
		 * the c-terminus residue */
		UseSequenceCriteria = true;
		...
		/* Require that included peptides have sequences
		   ending in one of the following residues */
		UseSequenceCriteria = true;
		SeqMustEndWithResidues = args[i+1];
		i++;
	    }
	    else if (args[i].equals("-Stn")) {
		/* Require that the included peptides follow one of
		...
		/* If IDs are mixed complete and incomplete sequences,
		...
		/* Require that the included +1 DTAs score less than
		 * this cutoff  */
		UseMaxXCorrs = true;
		MaxSingleChargeXCorr = new
		    Float(args[i+1]).floatValue();
		i++;
	    }
	    else if (args[i].equals("-X2")) {
		/* Require that the included +2 DTAs score less than
		 * this cutoff  */
		UseMaxXCorrs = true;
		MaxDoubleChargeXCorr = new
		    Float(args[i+1]).floatValue();
		i++;
	    }
	    else if (args[i].equals("-X3")) {
		/* Require that the included +3 DTAs score less than
		 * this cutoff  */
		UseMaxXCorrs = true;
		MaxTripleChargeXCorr = new
		    Float(args[i+1]).floatValue();
		i++;
	    }
	    else if (args[i].equals("-X4")) {
		/* Require that the included +4 DTAs score less than
		 * this cutoff  */
		UseMaxXCorrs = true;
		MaxQuadrupleChargeXCorr = new
		    Float(args[i+1]).floatValue();
		i++;
	    }
            else if (args[i].equals("-dm")) {
		...
		i++;
                ...
                /* Require that protein sequence
                   ends in one of the following residues */
                ProteinMustEndWithResidues = args[i+1];
                i++;
            }
            else if (args[i].equals("-StN")) {
                /* Require that protein sequence
                 * starts with one of the following residues */
    ...
    /* Check the passed protein to see if its name and description
     * match filters.  Return true if it's okay, and return false if
     * it's not.
     */
    ...
    /* Apply the spectrum-specific filters.  Return true if the
     * identification meets the criteria or false if it doesn't.
     */
    public boolean Allow(DTAFile TestSubject) {
	/* Evaluates an individual identification to determine if it
	...
	if (UseSequenceCriteria) {
	    ...
	    int          InsideCounter;
	    String       ShortSeq =
		TSequence.substring(0,TSequence.length() -1);
	    //Handle "Sic" option
	    for (Counter = 0; Counter <
		     SeqMustIncludeResidues.length(); Counter++) {
	    ...
	    //Handle "Sec" option
	    for (Counter = 0; Counter <
		     SeqMustExcludeBeforeC.length(); Counter++) {
		if (TSequence.indexOf(SeqMustExcludeBeforeC.charAt(Counter)) != -1)
		    PassesSequenceCriteria = false;
	    }
	    //Handle "Stc" option
	    if (SeqMustEndWithResidues.length() > 0) {
		if (SeqMustEndWithResidues.indexOf(TSequence.charAt(TSequence.length()-1)) == -1)
		    PassesSequenceCriteria = false;
	    }
	    //Handle "Stn" option
	    if (SeqMustComeAfterResidues.length() > 0) {
		if (SeqMustComeAfterResidues.indexOf(TestSubject.Sequence.charAt(0)) == -1)
		    PassesSequenceCriteria = false;
	    }
	    // Handle "Si2" option
		...
		HowMany = 0;
		for (InsideCounter = 0;
		     InsideCounter < TSequence.length();
		     InsideCounter++) {
		    if (TSequence.charAt(InsideCounter) ==
			SeqMustIncludeTwo.charAt(Counter))
			HowMany++;
		}
		if ( (HowMany < 2) ||
		     (HowMany > 2) )
		...
		for (InsideCounter = 0;
		     ...
		     InsideCounter++) {
		    if (TSequence.charAt(InsideCounter) ==
			SeqMustIncludeOne.charAt(Counter))
			HowMany++;
		}
		if ( (HowMany < 1) ||
		     (HowMany > 1) )
		...
		for (InsideCounter = 0;
		     ...
		     InsideCounter++) {
		    if (TSequence.charAt(InsideCounter) ==
			SeqMustIncludeNone.charAt(Counter))
			HowMany++;
		}
		if (HowMany > 0)
		    PassesSequenceCriteria = false;
		...
		boolean      CComplete = BetweenDots.charAt(BetweenDots.length()-1) != '-';
		...
		for (Counter = 0; Counter < TSequence.length() - 2; Counter++) {
			...
			if (TSequence.charAt(InsideCounter) != 'P') {
			    ...
			    if (TSequence.charAt(InsideCounter) == 'S' || TSequence.charAt(InsideCounter) == 'T') {
				FoundNGly = true;
			    }
			}
		    }
	...
	if (UseMaxXCorrs) {
	    PassesMaxXCorrs =
		( (TestSubject.ChargeState == 1) && (TestSubject.XCorr < this.MaxSingleChargeXCorr)  ) ||
	...
	if (UseXCorrCriteria) {
	    PassesXCorrCriteria = 
		( ( (TestSubject.ChargeState == 1) && (TestSubject.XCorr > this.MinSingleChargeXCorr)  ) ||
		  ( (TestSubject.ChargeState == 2) && (TestSubject.XCorr > this.MinDoubleChargeXCorr)  ) ||
		  ( (TestSubject.ChargeState == 3) && (TestSubject.XCorr > this.MinTripleChargeXCorr)  ) ||
		  ( (TestSubject.ChargeState >= 4) && (TestSubject.XCorr > this.MinQuadrupleChargeXCorr)  ) ) &&
		(TestSubject.DeltCN > this.MinDeltCN) ;
	}
	ObservedMZ = (TestSubject.PrecursorMass - 1 + TestSubject.ChargeState) / TestSubject.ChargeState;
	if (UseMinMZ) {
	...
	PassesOnMerits = (PassesSequenceCriteria && PassesMaxXCorrs && PassesMZ && PassesDeltaMass
			  && PassesXCorrCriteria && PassesFilenameFilter
			  ...
			  && (
			      ...
			      ( (HandleModified == 2) && !TestSubject.Modified) ||
			      (HandleModified ==1) )
			  ...
			  && (TestSubject.Sp < this.MaximumSp)
			  && (TestSubject.SpScore > this.MinSpScore)
			  );
	...
	case 1:
	    ...
	    default:
		System.out.println("Validation went awry for " + TestSubject.FileName);
		return PassesOnMerits;
	    }
	case 2:
	    switch (TestSubject.Validated) {
    ...
    /* Create a string announcing the current settings for each of the
     * criteria.
     */
	...
	if (LocusValidation == 1)
	    ...
	    LocValid = "+N -YMU";
	return (
		RowStart + new Boolean(UseCriteria).toString() + Separator + "Use criteria" + RowEnd +
		RowStart + new Float(MinPepConf).toString() +
                Separator + "Minimum peptide probability" + RowEnd +
                RowStart + new Float(MaxPepFP).toString() +
                Separator + "Peptide global false discovery rate" + RowEnd +
                RowStart + new Float(MinProtConf).toString() +
                Separator + "Minimum protein probability" + RowEnd +
                RowStart + new Float(MaxProtFP).toString() +
		Separator + "Protein false discovery rate" + RowEnd +
		(DisplayXCorrCriteria ?
			(RowStart + new Float(MinSingleChargeXCorr).toString() +
			Separator + "Minimum +1 XCorr" + RowEnd +
			RowStart + new Float(MinDoubleChargeXCorr).toString() +
			Separator + "Minimum +2 XCorr" + RowEnd +
			RowStart + new Float(MinTripleChargeXCorr).toString() +
			Separator + "Minimum +3 XCorr" + RowEnd +
			RowStart + new Float(MinQuadrupleChargeXCorr).toString() +
                	Separator + "Minimum +4 XCorr" + RowEnd +
			RowStart + new Float(MinDeltCN).toString() +
			Separator + "Minimum DeltCN" + RowEnd) : "") +
		(UseMaxXCorrs ? (RowStart + MaxSingleChargeXCorr +
				 Separator + "Maximum +1 XCorr" + RowEnd +
				 RowStart + MaxDoubleChargeXCorr +
				 Separator + "Maximum +2 XCorr" + RowEnd +
				 RowStart + MaxTripleChargeXCorr +
				 Separator + "Maximum +3 XCorr" + RowEnd +
				 RowStart + MaxQuadrupleChargeXCorr +
                                 Separator + "Maximum +4 XCorr" + RowEnd) : "") +
		RowStart + new Integer(MinChargeState).toString() +
		Separator + "Minimum charge state" + RowEnd +
		RowStart + new Integer(MaxChargeState).toString() +
		Separator + "Maximum charge state" + RowEnd +
		RowStart + new Float(MinIonProportion).toString() +
		Separator + "Minimum ion proportion" + RowEnd +
		RowStart + new Integer(MaximumSp).toString() +
		Separator + "Maximum Sp rank" + RowEnd +
		RowStart + new Float(MinSpScore).toString() +
		Separator + "Minimum Sp score" + RowEnd +
                (UseMinDeltaMass ? (RowStart + MinDeltaMassPPM + Separator + "Minimum mass deviation (ppm)" + RowEnd) : "") +
                (UseMaxDeltaMass ? (RowStart + MaxDeltaMassPPM + Separator + "Maximum mass deviation (ppm)" + RowEnd) : "") +
                (DisplayDeltaMass ? (RowStart + Isotopes + Separator + "Define delta mass with respect to nearest isotope" + RowEnd) : "") +
		RowStart + Mods + Separator + "Modified peptide inclusion" + RowEnd
		+ RowStart + Tryptic + Separator + "Tryptic status requirement" + RowEnd
		+ RowStart + PermitAmbiguous + Separator + "Multiple, ambiguous IDs allowed" + RowEnd
		+ RowStart + PepValid + Separator + "Peptide validation handling" + RowEnd
		+ (UseSequenceCriteria ? (RowStart + SeqMustIncludeResidues + Separator +
					  "Sequences must include all of" + RowEnd +
					  RowStart + SeqMustIncludePattern + Separator +
					  "Sequences must include pattern" + RowEnd +
					  RowStart + SeqMustExcludeBeforeC + Separator +
					  "Sequences can contain none of" + RowEnd +
					  RowStart + SeqMustComeAfterResidues + Separator +
					  ...
					  "Minimum sequence length" + RowEnd +
					  RowStart + SeqMaxLength + Separator +
					  "Maximum sequence length" + RowEnd +
					  RowStart + SequenceCompleteness + Separator +
					  "Sequence completeness required" + RowEnd) : "")
		+ (UseMinMZ ? (RowStart + MinMZ + Separator + "Minimum peptide m/z" + RowEnd) : "")
		+ (UseMaxMZ ? (RowStart + MaxMZ + Separator + "Maximum peptide m/z" + RowEnd) : "")
		+ RowStart + Purge + Separator + "Purge duplicate peptides by protein" + RowEnd
		+ RowStart + new Boolean(IncludeOnlyUniques).toString() + Separator + "Include only loci with unique peptide" + RowEnd
		+ RowStart + new Boolean(RemoveSubsets).toString() + Separator + "Remove subset proteins" + RowEnd
		+ RowStart + LocValid + Separator + "Locus validation handling" + RowEnd
		+ (NameExcludePattern.length() > 0 ? (RowStart + NameExcludePattern +
						    Separator + "Exclude protein names matching" + RowEnd) : "")
		+ (NameIncludePattern.length() > 0 ? (RowStart + NameIncludePattern +
						    Separator + "Include only protein names matching" + RowEnd) : "")
		+ (DescripExcludePattern.length() > 0 ? (RowStart + DescripExcludePattern +
						    Separator + "Exclude protein descriptions matching" + RowEnd) : "")
		+ (DescripIncludePattern.length() > 0 ? (RowStart + DescripIncludePattern +
						    Separator + "Include only protein descriptions matching" + RowEnd) : "")
		+ (UseMinMW ? (RowStart + MinMW + Separator + "Minimum protein molecular weight" + RowEnd): "")
		+ (UseMaxMW ? (RowStart + MaxMW + Separator + "Maximum protein molecular weight" + RowEnd): "")
		+ (UseMinPI ? (RowStart + MinPI + Separator + "Minimum protein isoelectric point" + RowEnd): "")
		+ (UseMaxPI ? (RowStart + MaxPI + Separator + "Maximum protein isoelectric point" + RowEnd): "")
		+ RowStart + new Integer(MinModPepsPerLocus).toString() + Separator + "Minimum modified peptides per locus" + RowEnd
		+ RowStart + new Integer(MinPepsPerLocus).toString() + Separator + "Minimum peptides per locus" + RowEnd
		);
    }

    /* Create a string announcing the current settings for each of the
     * criteria.  The formatting is XML.  NOTE: THIS IS OUT-OF-DATE!
     */
	...
	if (HandleModified == 0)
	    ...
	    Mods = "Exclude";
	return (
		"\t<DTASelect:UseCriteria>" + new Boolean(UseCriteria).toString() + "</DTASelect:UseCriteria>\n"
		+ "\t<DTASelect:Min1XCorr>" + new Float(MinSingleChargeXCorr).toString() + "</DTASelect:Min1XCorr>\n"
		+ "\t<DTASelect:Min2XCorr>" + new Float(MinDoubleChargeXCorr).toString() + "</DTASelect:Min2XCorr>\n"
		+ "\t<DTASelect:Min3XCorr>" + new Float(MinTripleChargeXCorr).toString() + "</DTASelect:Min3XCorr>\n"
                + "\t<DTASelect:Min4XCorr>" + new Float(MinQuadrupleChargeXCorr).toString() + "</DTASelect:Min4XCorr>\n"
		+ "\t<DTASelect:Max1XCorr>" + new Float(MaxSingleChargeXCorr).toString() + "</DTASelect:Max1XCorr>\n"
		+ "\t<DTASelect:Max2XCorr>" + new Float(MaxDoubleChargeXCorr).toString() + "</DTASelect:Max2XCorr>\n"
		+ "\t<DTASelect:Max3XCorr>" + new Float(MaxTripleChargeXCorr).toString() + "</DTASelect:Max3XCorr>\n"
                + "\t<DTASelect:Max4XCorr>" + new Float(MaxQuadrupleChargeXCorr).toString() + "</DTASelect:Max4XCorr>\n"
		+ "\t<DTASelect:MinDeltCN>" + new Float(MinDeltCN).toString() + "</DTASelect:MinDeltCN>\n"
                + "\t<DTASelect:MinPepConf>" + new Float(MinPepConf).toString() + "</DTASelect:MinPepConf>\n"
		+ "\t<DTASelect:MaxPepFP>" + new Float(MaxPepFP).toString() + "</DTASelect:MaxPepFP>\n"
                + "\t<DTASelect:MinProtConf>" + new Float(MinProtConf).toString() + "</DTASelect:MinProtConf>\n"
                + "\t<DTASelect:MaxProtFP>" + new Float(MaxProtFP).toString() + "</DTASelect:MaxProtFP>\n"
		+ "\t<DTASelect:MinChargeState>" + new Integer(MinChargeState).toString() + "</DTASelect:MinChargeState>\n"
		+ "\t<DTASelect:MaxChargeState>" + new Integer(MaxChargeState).toString() + "</DTASelect:MaxChargeState>\n"
		+ "\t<DTASelect:MinIonProportion>" + new Float(MinIonProportion).toString() + "</DTASelect:MinIonProportion>\n"
		+ "\t<DTASelect:MaxSpRank>" + new Integer(MaximumSp).toString() + "</DTASelect:MaxSpRank>\n"
		+ "\t<DTASelect:SetDeltCN>" + new Integer(SetDeltCN).toString()
+ "</DTASelect:SetDeltCN>\n"
		+ "\t<DTASelect:ModifiedInclusion>" + Mods + "</DTASelect:ModifiedInclusion>\n"
		+ "\t<DTASelect:TrypticInclusion>" + Tryptic + "</DTASelect:TrypticInclusion>\n"
		+ "\t<DTASelect:SequenceIncRes>" + SeqMustIncludeResidues + "</DTASelect:SequenceIncRes>\n"
		+ "\t<DTASelect:SequenceIncPat>" + SeqMustIncludePattern + "</DTASelect:SequenceIncPat>\n"
		+ "\t<DTASelect:SequenceExcRes>" + SeqMustExcludeBeforeC + "</DTASelect:SequenceExcRes>\n"
		+ "\t<DTASelect:SequenceFollows>" + SeqMustComeAfterResidues + "</DTASelect:SequenceFollows>\n"
		+ "\t<DTASelect:SequenceEndsWith>" + SeqMustEndWithResidues + "</DTASelect:SequenceEndsWith>\n"
		+ "\t<DTASelect:PurgeDuplicates>" + Purge + "</DTASelect:PurgeDuplicates>\n"
		+ "\t<DTASelect:OnlyUniques>" + new Boolean(IncludeOnlyUniques).toString() + "</DTASelect:OnlyUniques>\n"
		+ "\t<DTASelect:SubsetsRemoved>" + new Boolean(RemoveSubsets).toString() + "</DTASelect:SubsetsRemoved>\n"
		+ "\t<DTASelect:ExcludeName>" + NameExcludePattern + "</DTASelect:ExcludeName>\n"
		+ "\t<DTASelect:IncludeName>" + NameIncludePattern + "</DTASelect:IncludeName>\n"
		+ "\t<DTASelect:ExcludeDescrip>" + DescripExcludePattern + "</DTASelect:ExcludeDescrip>\n"
		+ "\t<DTASelect:IncludeDescrip>" + DescripIncludePattern + "</DTASelect:IncludeDescrip>\n"
		+ "\t<DTASelect:MinModPeptidesPerLocus>" + new Integer(MinModPepsPerLocus).toString() + "</DTASelect:MinModPeptidesPerLocus>\n"
		+ "\t<DTASelect:MinPeptidesPerLocus>" + new Integer(MinPepsPerLocus).toString() + "</DTASelect:MinPeptidesPerLocus>\n"
		);
    }

    class SpectrumGUI extends Frame {
	private Checkbox      n = new Checkbox("Use Criteria", UseCriteria);
	private TextField     c = new TextField(new Integer(MinChargeState).toString());
	private TextField     C = new TextField(new Integer(MaxChargeState).toString());
	private TextField     s = new TextField(new Integer(MaximumSp).toString());
	private TextField     z1 = new TextField(new Float(MinSingleChargeXCorr).toString());
	private TextField     z2 = new TextField(new Float(MinDoubleChargeXCorr).toString());
	private TextField     z3 = new TextField(new Float(MinTripleChargeXCorr).toString());
	private TextField     z4 = new TextField(new Float(MinQuadrupleChargeXCorr).toString());
	...
	private Checkbox      m0 = new Checkbox("Require peptides to be modified", m, false);
	private Checkbox      m1 = new Checkbox("Include peptides regardless of modification", m, true);
	private Checkbox      m2 = new Checkbox("Exclude modified peptides", m, false);
	private CheckboxGroup y = new CheckboxGroup();
	private Checkbox      y0 = new Checkbox("Include peptides regardless of tryptic status", y, true);
	private Checkbox      y1 = new Checkbox("Include only half- or fully tryptic peptides", y, false);
	private Checkbox      y2 = new Checkbox("Include only fully tryptic peptides", y, false);
	...
	private TextField     X1 = new TextField(new Float(MaxSingleChargeXCorr).toString());
	private TextField     X2 = new TextField(new Float(MaxDoubleChargeXCorr).toString());
	private TextField     X3 = new TextField(new Float(MaxTripleChargeXCorr).toString());
	private TextField     X4 = new TextField(new Float(MaxQuadrupleChargeXCorr).toString());
	...
	public ExtendedGUI() {
	    ...
	    add(X4);
	    NormalConstraints.gridx = 1;
	    TempLabel = new Label("Must include all these residues before C terminus");
	    Layout.setConstraints(TempLabel, NormalConstraints);
	    add(TempLabel);
	    TempLabel = new Label("Must include this pattern");
	    Layout.setConstraints(TempLabel, NormalConstraints);
	    add(TempLabel);
	    TempLabel = new Label("Must exclude all these residues before C terminus");
	    Layout.setConstraints(TempLabel, NormalConstraints);
	    add(TempLabel);
	    TempLabel = new Label("Must be preceded by one of these residues");
	    Layout.setConstraints(TempLabel, NormalConstraints);
	    add(TempLabel);
	    TempLabel = new Label("Must have one of these residues at C terminus");
	    Layout.setConstraints(TempLabel, NormalConstraints);
	    add(TempLabel);
	...
	private TextField     p = new TextField(new Integer(MinPepsPerLocus).toString());
	private Checkbox      u = new Checkbox("Include only loci with uniquely matching peptides");
	private CheckboxGroup t = new CheckboxGroup();
	private Checkbox      t0 = new Checkbox("Do not purge duplicate spectra for each sequence", t, false);
	private Checkbox      t1 = new Checkbox("Purge duplicate spectra in each salt step", t, false);
	private Checkbox      t2 = new Checkbox("Purge duplicate spectra on basis of XCorr", t, true);
	private Checkbox      compress = new Checkbox("Compress .dta files to create IDX and SPM files", false);
	private Checkbox      copy = new Checkbox("Create copy script", false);
	private Checkbox      GUI = new Checkbox("Show GUI of results", true);
	private Checkbox      dot = new Checkbox("Run on .dta files in current directory only", false);
	...
	public LocusGUI() {
	    ...
	    NormalConstraints.gridx = 1;
	    TempLabel = new Label("Show all loci with peptides appearing this many times");
	    Layout.setConstraints(TempLabel, NormalConstraints);
	    add(TempLabel);
