...
public class DTASelect {
	...
	System.out.println(Protein.Version() + ", a program for selecting specific SEQUEST results");
	System.out.println("Copyright 2001, Yates Lab, The Scripps Research Institute");
	...
	System.out.println("-y 0\tInclude peptides regardless of cleavage status");
	System.out.println("-y 1\tInclude only half- or fully cleaved peptides");
	System.out.println("-y 2\tInclude only fully cleaved peptides");
	System.out.println("--mc #\tSet minimum number of missed tryptic sites");
        System.out.println("--MC #\tSet maximum number of missed tryptic sites");
	System.out.println("-a true | false\tAllow peptides with ambiguous IDs");
	...
	System.out.println("-v 3\tKeep Y and M peptides, discard N and U peptides");
	System.out.println();
	...
	System.out.println("-1 #\tSet lowest +1 XCorr");
        ...
        System.out.println("-i #\tSet lowest proportion of fragment ions observed");
        System.out.println("-s #\tSet maximum Sp ranking");
        System.out.println("-S #\tSet minimum Sp score");
	System.out.println("-Sic $\tSequences must contain all of these residues");
	System.out.println("-Sip $\tSequences must contain this pattern");
	System.out.println("-Sec $\tSequences must not contain any of these residues");
	System.out.println("-Si2 $\tSequences must contain exactly two of these residues");
	System.out.println("-Si1 $\tSequences must contain exactly one of these residues");
	...
	System.out.println("-Smn #\tSequence must be at least this long (default = 6)");
	System.out.println("-Smx #\tSequence must be no longer than this (default = 100)");
	System.out.println("-Ser #\tSequence must have this many complete ends");
	System.out.println("-Ngly 0\tFilter out N-linked glycosylated peptides");
	System.out.println("-Ngly 1\tKeep only N-linked glycosylated peptides");
	System.out.println("-file $\tOnly show spectra from MS2 files matching this string");
	System.out.println();
	System.out.println("LOCUS FILTERS:");
	System.out.println("-t 0\tShow all spectra for each sequence");
	System.out.println("-t 1\tShow only one spectrum per salt step for each sequence");
	...
	System.out.println("-V 3\tKeep Y and M proteins, discard N and U proteins");
	System.out.println("-u\tInclude only loci with uniquely matching peptides");
	System.out.println("-in\tInclude proteins that are subsets of others");
	System.out.println("-e $\tRemove proteins with IDs matching this string");
	...
	System.out.println("-StN $\tInclude only proteins that start with one of these residues");
	System.out.println("-StC $\tInclude only proteins that end with one of these residues");
	System.out.println("-E $\tInclude only proteins with IDs matching this string");
	System.out.println("-l $\tRemove proteins with descriptions including this word");
	System.out.println("-L $\tInclude only proteins with descriptions including this word");
	System.out.println("-M #\tSet minimum modified peptides per locus criterion");
	System.out.println("-tp #\tSet minimum fully cleaved peptides per locus criterion");
	System.out.println("-ty #\tSet minimum cleavage status per locus criterion");
	...
	System.out.println("-tpr #\tSet minimum probability per locus criterion");
	System.out.println("-tDM #\tSet maximum PPM delta mass per locus criterion");
	System.out.println("-p #\tSet minimum peptides per locus criterion");
	System.out.println("-N #\tSet minimum spectral count per locus criterion");
	System.out.println();
        System.out.println("OPTIONS:");
	System.out.println("--short\t\tUse actual (short form) accession numbers");
        System.out.println("--dcn 0\t\tDefine DeltCN with respect to the second-best peptide");
        System.out.println("--dcn 1\t\tDefine DeltCN with respect to the next best DIFFERENT peptide");
        System.out.println("--iso\t\tDefine delta mass with respect to nearest isotope");
        System.out.println("--noiso\t\tDefine delta mass with respect to monoisotopic/average mass");
	...
	System.out.println("--before $\tEnzyme cuts before one of these residues");
	System.out.println("--after $\tEnzyme cuts after one of these residues");
        System.out.println();
        System.out.println("STATISTICS:");
	System.out.println("--decoy $\tDatabase protein decoy label (default: \"Reverse_\")");
	System.out.println("--hidedecoy\tHide decoy database hits");
	System.out.println("--trueprot $\tTrue proteins have IDs matching this string");
	System.out.println("--nostats\tNo statistics");
//        System.out.println("--noemp\t\tNo empirical coefficients used");
	System.out.println("--cmax #\tMaximum charge state for statistics options");
        System.out.println("--raw\t\tUse raw primary score (XCorr) values, no normalization");
	System.out.println("--nochgstat\tDon't use models for each charge state");
	System.out.println("--trypstat\tUse cleavage status when calculating probabilities");
	System.out.println("--modstat\tUse separate models for modified peptides");
...
//        System.out.println("--fit 2\t\tKernel function-based statistics");
	System.out.println("--fptol #\tInitial FP rate used in determining linear coefficients");
	System.out.println("--plot 0\tDo not plot score distributions (default)");
	...
	System.out.println("--plot 3\tAlso plot normal and decoy score histograms");
        System.out.println();
	...
	System.out.println("--HPLC\t\tDisplay HPLC index adjusted for pH3.4 conditions");
	System.out.println("--seqpos\tDisplay sequence position in the DTASelect-filter.txt file");
	System.out.println("--extra\t\tOutput extra columns in DTASelect-filter.txt and DB-Peptides.txt files");
	...
	System.out.println("--GUI\t\tReport through GUI instead of output files");
	System.out.println("--BE\t\tDisplay Bird's Eye View of proteins");
	System.out.println("--compress\tCreate .IDX and .SPM files from spectra");
	...
	System.out.println("--Mascot\tExpect Mascot output rather than SEQUEST");
	...
	System.out.println("--path $\tSpecify location of the DTASelect folder");
   	System.out.println();
	...
	// Set criteria according to DTASelect.params
	ParamsFileOptions = AppObject.Data.Cutoffs.ProcessParamsFile();
	if (ParamsFileOptions != null) {
	    for(i = 0; i < ParamsFileOptions.length; i++) {
	...
	AppObject.Data.Cutoffs.SetCriteria(args);
	...
	if (AppObject.Data.Cutoffs.UseGUIInstead || Protein.PrintTest()) {
	    ...
	    AppObject.ReadAndPreprocessProteins();
		...
		//Report results
		AppObject.Data.PrintReports(AppObject.CommandLineOptions, AppObject.Configuration, "DTASelect");
	    }
	    System.out.println("DTASelect is completed.");
	}
	else {
	    System.out.println("I can't write DTASelect.html in this directory.");
	}
    }
	...
	try {
	    ...
	    RAHandle = new RandomAccessFile(new File(CurrentDirectory, "DTASelect.SPM"), "rw");
	    for (DirIterator = 0; DirIterator < SubDirList.length; DirIterator++) {
		CurrentSubDir = new File(CurrentDirectory, SubDirList[DirIterator]);
		DirectoryListing = CurrentSubDir.list(FilenameFilter);
		BubbleSort(DirectoryListing);
		Index.AddSubdir(SubDirList[DirIterator]);
		//Let users know that we're starting to read files in this subdirectory
		System.out.println("Now reading spectra in " + SubDirList[DirIterator]);
		for (FileIterator = 0; FileIterator < DirectoryListing.length; FileIterator++) {
		    Parser = new StringTokenizer(DirectoryListing[FileIterator],".");
		    Parser.nextToken();
		    CurrentLowScan = new Short(Parser.nextToken()).shortValue();
		    CurrentHighScan = new Short(Parser.nextToken()).shortValue();
		    if ( (CurrentLowScan != LastLowScan) || (CurrentHighScan != LastHighScan) ) {
		...
		System.out.println("Compressing spectra for " + SubDirList[DirIterator]);
	...
	/* STAGE 1: Read files from disk whether from .out files or
	 * from .txt file.  If DTASelect.txt exists, read it.
	 * Otherwise resort to parsing the .out files and match their
	 * sequences to loci listed in FASTA database.
	 */
	...
	if (DTASelectTXT.exists()) {
		...
		if (Data.SequestParams == null) {
		    try {
			//Read sequest.params file
			Data.SequestParams = ParamsFile.ReadFile(CurrentDirectory);
		    }
		    catch(IOException failure) {
			System.out.println("Couldn't read sequest.params file.");
			System.exit(0);
		    }
		}
	    }   
	    catch(IOException failure) {
		System.out.println("Couldn't read DTASelect.txt file.  You might try deleting it and re-running.");
		System.exit(0);
	    }
	}
	/* If DTASelect.txt doesn't exist, read information in from
	 * .out files, sequest.params file, and database. */
	else {
	    /*If DTASelect.txt doesn't exist, read data:
	      0) If --Mascot, read .dat files
	      1) If .sqt files exist, read them.
	      2) If no .sqt, read:
	      A) .outs in current directory (if --here is specified)
	      B) .outs in subdirectories
	    */
	    if (Data.Cutoffs.UseMascotOutput) {
		CurrentSubDir = CurrentDirectory;
		// Check to see if DTASelect.params lists the .dats to be included
		try {
		    File                ParamsFile = new File(CurrentDirectory, "DTASelect.params");
		    //Read a DTASelect.params file, grabbing the DAT list from there.
			...
			if (ParamsFile.canRead()) {
				...
				while ( (LineBuffer != null) &&
					(!LineBuffer.startsWith("[DAT List]")) ){
				    LineBuffer = Incoming.readLine();
				}
		    ...
		    //If there's a failure, punt, reading all .dats in current directory
		    DirectoryListing = null;
		}
		...
		if (DirectoryListing.length > 0) {
		    ...
		    // Get the search configuration and database from the first file.
		    Data.SequestParams = ParamsFile.ReadMascotConfig(DirectoryListing[0],
								     Configuration);
		    Data.IDFileFormat = "Mascot";
		    Data.SQTGenerator = "Mascot";
		    System.out.println("Now reading Mascot results in DAT files:");
		    Buffer = OutFileList;
		    for (FileIterator = 0; FileIterator < DirectoryListing.length; FileIterator++) {
			System.out.println("Now reading " + DirectoryListing[FileIterator]);
			// Function returns numbers of SEQUEST .outs represented by this file
			try {
			    SQTCount = OUTFile.ReadMascotDAT(DirectoryListing[FileIterator],
							     Buffer, Configuration);
			    System.out.println("\t" + SQTCount + " identifications read.                              ");
			    ...
			    System.out.println("Error while reading .dat files.");
			    System.out.println(failure.getMessage());
			    System.out.println(failure.getLocalizedMessage());
			    System.out.println(failure.toString());
			    System.exit(0);
		...
		if (!Test.canRead()) {
		    System.out.println("Couldn't find database " + Data.SequestParams.DBName);
		    System.exit(0);
		}
		...
		if (DirectoryListing.length > 0) {
		    ...
		    Buffer = OutFileList;
		    try {
			BufferedReader   Incoming = new BufferedReader(new FileReader(new File(DirectoryListing[0])));
			...
			while ( (WholeLine != null) &&
				...
				if (LineBuffer.startsWith("SEQUEST") ||
					...
					else if (LineBuffer.equals("SQTGeneratorVersion") &&
						 (Parser.hasMoreTokens()))
					    Data.SQTGeneratorVersion = Parser.nextToken();
				    }
			...
			Incoming.close();
			System.out.println("Identifications come from " + Data.SQTGenerator +
					   ", version " + Data.SQTGeneratorVersion);
		    }
		    catch(IOException failure) {
			System.out.println("Error while attempting to identify program used to make SQT.");
		    }
		    for (FileIterator = 0; FileIterator < DirectoryListing.length; FileIterator++) {
			System.out.println("Now reading " + DirectoryListing[FileIterator]);
			// Function returns numbers of SEQUEST .outs represented by this file
			try {
			    SQTCount = OUTFile.ReadSQT(CurrentSubDir, DirectoryListing[FileIterator], Buffer, Configuration, Data.Cutoffs);
			    System.out.println("\t" + SQTCount + " identifications read.                              ");
			    Data.UnfilteredOUTCount += SQTCount;
			    while (Buffer.Next != null) {
				Buffer = Buffer.Next;
			    ...
			    System.out.println("Error while reading .sqt files.");
			    System.out.println(failure.getMessage());
			    System.out.println(failure.getLocalizedMessage());
			    System.out.println(failure.toString());
			    System.exit(0);
		    ...
		    try {
			/* This if is triggered when the user has specified
			 * the -. (current directory only) option. */
			if (Data.Cutoffs.LookInCurrentDirectory) {
			    // Examine only this directory for .out files
			    System.out.print("Now reading in current directory: ");
			    DirectoryListing = CurrentDirectory.list(FilenameFilter);
			    Data.UnfilteredOUTCount += DirectoryListing.length;
			    System.out.println(new Integer(DirectoryListing.length).toString() + " files");
			    //Loop through all the .outs in the current directory
			    for (FileIterator = 0; FileIterator < DirectoryListing.length; FileIterator++) {
				OUTFile.GetVersion(CurrentSubDir, DirectoryListing[0], Data);
				//Create an OUTFile object for each .out file in this subdirectory
				Buffer = OUTFile.ReadOUT(CurrentSubDir, DirectoryListing[FileIterator], true, Configuration, Data.Cutoffs);
				if (Buffer != null) {
				//Since some .out files are multiple loci, a linked list
				//might have been returned rather than a single object.
			 ...
			 * subdirectories of the current directory are
			 * searched for .out files, and a count for each
			 * directory is printed as the reading proceeds.  If
			 * the SEQUEST box failed to process all DTAs, a
			 * directory of zero files should flag the user that
			 * something's gone wrong. */
			else {
			    boolean   DontKnowVersionYet = true;
			    // Loop through each subdirectory in search of .out files
			    for (DirIterator = 0; DirIterator < SubDirList.length; DirIterator++) {
				CurrentSubDir = new File(CurrentDirectory, SubDirList[DirIterator]);
				DirectoryListing = CurrentSubDir.list(FilenameFilter);
				//Let users know that we're starting to read files in this subdirectory
				System.out.print("Now reading in " + SubDirList[DirIterator] + ": ");
				Data.UnfilteredOUTCount += DirectoryListing.length;
				System.out.println(new Integer(DirectoryListing.length).toString() + " files");
				//Loop through all the .outs in this subdirectory
				for (FileIterator = 0; FileIterator < DirectoryListing.length; FileIterator++) {
				    if (DontKnowVersionYet) {
					OUTFile.GetVersion(CurrentSubDir, DirectoryListing[FileIterator], Data);
	    ...
	    System.out.println("Finished reading database search identifications.");
		...
		int ChargeState;
		byte Tryptic;
		int NChargeStates = (Data.Cutoffs.UseChargeState ? Data.Cutoffs.MaxStatisticsCharge : 1);
		...
		for (iCharge = 0; iCharge < NChargeStates; iCharge++) {
                    Tryptic = 0; Modified = false;
               	    System.out.println("Now computing linear coefficients for the following subset:");
		    System.out.println("\tCharge state:\t\t\t" + (Data.Cutoffs.UseChargeState ? ChargeState : "all"));
	       	    PeptideConfidence = new LDAConfidence(OutFileList, Data.Cutoffs, ChargeState, Modified, Tryptic, true);
	       	    PeptideConfidence.ComputeConfidence();
		    LinearCoefficients = new double[PeptideConfidence.NDim];
		    PeptideConfidence.SaveLinearCoefficients(LinearCoefficients);
	       	    ...
	       	    PeptideConfidence = null;
                    ...
                    for (iTryptic = 0; iTryptic < NTryptic; iTryptic++) {
                        ...
                        for (iModified = 0; iModified < NModified; iModified++) {
                            System.out.println("Now computing peptide confidence levels for the following subset:");
                            System.out.println("\tCharge state:\t\t\t" + (Data.Cutoffs.UseChargeState ? ChargeState : "all"));
                            System.out.println("\tEnzyme status:\t\t\t" + (Data.Cutoffs.UseTrypticInfo ? Tryptic : "all"));
                            System.out.println("\tModified status:\t\t" + (Data.Cutoffs.UseModStats ? Modified : "all"));
                            PeptideConfidence = new LDAConfidence(OutFileList, Data.Cutoffs, ChargeState, Modified, Tryptic, false);
			    PeptideConfidence.GetLinearCoefficients(LinearCoefficients);
                            PeptideConfidence.ComputeConfidence();
	    ...
	    /* Match up a database sequence and gene name with each
	     * identified locus.  Determine where individual peptides
	     * align to protein sequence.  Calculate MW and pI for
	     * each protein. */
	    try {
		Data.LocusList.LookUpLoci(Data.SequestParams, Configuration, Data.Cutoffs);
	    }
	    catch (IOException failure) {
...
/*            if (Data.Cutoffs.UseStatistics) {
	        ...
	        ProteinConfidence = new LDAConfidence(Data.LocusList, Data.Cutoffs);
	        ProteinConfidence.OldComputeConfidence();
	        ProteinConfidence.ReportScores(Data.LocusList);
	        ProteinConfidence = null;
	    } */
	    //Write DTASelect.txt database to current directory
	    System.out.println("Now writing information to DTASelect.txt.            ");
	    Data.SourceDirectory = System.getProperty("user.dir");
	    Data.CutoffsString = CommandLineOptions;
	    Data.PrintTXT();
	    // Data.PrintTXT(Data.Cutoffs, CommandLineOptions, "DTASelect.txt", Data.SequestParams);
	    System.out.println("Finished writing information to DTASelect.txt.");
	}
    }

    /* This filter allows one to list subdirectories found in another
     * directory.  It is used to list directories containing .out
     * files.
     */
