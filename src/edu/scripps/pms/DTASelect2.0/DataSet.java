...
public class DataSet {
	...
	else if (SQTGenerator.equals("SEQUEST") && SQTGeneratorVersion.equals("3.0")) {
	    hSpScore = "ZScore";
	}
    }

    /* Caution!  This function is used by both Contrast and
       DTASelect. Print reports on the remaining proteins in this
       dataset */
	...
	/* If the new CGIs are used, we need to rerun the sequence
	   coverage generator because we used the old consensus URL to
	   detect identicals. */
	...
	if (Cutoffs.PrintXML) {
	    ...
	    LocusList.PrintXML(Cutoffs,
				   CommandLineOptions,
				   SequestParams,
				   RootName + "-filter.xml",
				   this.SourceDirectory);
	}
	/* Users may indicate their desire to have a script or batch
	 * file created for them that will allow them to copy the
	 * selected .dta and .out files to a separate directory.  If
	 * they have used the --copy parameter, this script will be
	 * sent out to the console.
	 */
	...
	System.out.println("\tCreating DTASelect.html and DTASelect-filter.txt...");
    ...
    /* Print an HTML web page and TXT file from the loci and DTAFiles
     * remaining in memory after the filtering process.  NOTE:
     * after this funcion is called, the integrity of the protein list
     * is destroyed.
     */
    public void PrintOutput(String CommandLineOptions, String FileName,
			    IniFile Config, String DBPrefix) {
	...
	String                  NewShow = "</td><td><a target=\"Win1\" HREF=\"" +
	    Config.SQTDisplay + "?Dr=" +
	    (this.Cutoffs.UseCustomPath ? this.Cutoffs.CustomPath : this.SourceDirectory) + "&Da=";
	String                  BLAST1 = "</td><td><a target=\"Win1\" href=\"" +
	    Config.BLAST;
	String                  BLAST2 = Config.BLASTArgs + "\">";
	String                  ProtEval = "<a target=\"Win1\" href=\"" +
	    Config.ProtValidation +
	    "?" + URLEncoder.encode(this.Cutoffs.UseCustomPath ? this.Cutoffs.CustomPath : this.SourceDirectory) + "&";
	...
	StaticModString = StaticModCGI.toString() +
	    "&Pep=";
	try {
	    ...
	    Outgoing.write((this.Cutoffs.UseCustomPath ? this.Cutoffs.CustomPath : this.SourceDirectory) + "<BR>\n");
	    Outgoing.write(SequestParams.DBName + "<BR>\n");
	    Outgoing.write(this.SQTGenerator + " " +
			   this.SQTGeneratorVersion + " in " +
			   this.IDFileFormat + " format.<BR>\n");
	    Outgoing.write(CommandLineOptions + "<BR>\n");
	    ...
	    Outgoing.write(SequestParams.GetHTMLMods() + newline);
	    ...
	    OutgoingTXT.write((this.Cutoffs.UseCustomPath ? this.Cutoffs.CustomPath : this.SourceDirectory) + newline);
	    OutgoingTXT.write(SequestParams.DBName + newline);
	    OutgoingTXT.write(this.SQTGenerator + " " +
			   this.SQTGeneratorVersion + " in " +
			   this.IDFileFormat + " format.\n");
	    OutgoingTXT.write(CommandLineOptions + newline);
	    //Write the criteria in effect for this run
	    OutgoingTXT.write(Cutoffs.PrintCriteria("", newline, tab) + newline);
	    OutgoingTXT.write("Locus\tSequence Count\tSpectrum Count\tSequence Coverage\tLength\tMolWt\tpI\tValidation Status\t");
	    ...
	    Outgoing.write("<TD>Descriptive Name</td></tr></table>\n");
	    ...
	    OutgoingTXT.write("Descriptive Name\nUnique\tFileName\t" +
                                  this.hXCorr + tab + this.hDeltCN + tab +
                                  "Conf%\tM+H+\t" + this.hCalcPreMass + (Cutoffs.DisplayDeltaMass && Cutoffs.ExtraColumns ? "\tPPM" : "") +
                                  "\tTotalIntensity\t" + this.hSpR + tab +
                                  this.hSpScore + 
	    ...
	    Counter = 0; DecoyCounter = 0; ForwardCounter = 0;
		...
		if (Cutoffs.UseClassifications) {
			...
			CurrentClassification = Classifieds.GetClass(MatchRunner.Classification);
		    }
		    else {
			// If we were already on a class, but this one doesn't match
			if (LastClassification.Identifier != MatchRunner.Classification) {
			    CurrentClassification = Classifieds.GetClass(MatchRunner.Classification);
			}
		    }
		    // Only print a new label if this is a change of classification
			...
			if (CurrentClassification != null) {
			    Outgoing.write("<h2><a name=\"Class" +
					   ...
					   "</h2></table>\n<table>");
			    OutgoingTXT.write("\nClass\t" + LastClassification.Descriptor);
			}
			else {
		...
		while (MatchRunner != null) {
		    ...
		    Outgoing.write(
				   "<TR><TD>" +
				   ProtEval + URLEncoder.encode(MatchRunner.Locus) + "\">" +
				   new Character(MatchRunner.Validated).toString() +
				   "</a>" +
				   CellSep +
				   "<em><a name=\"" +
				   MatchRunner.Locus + "\">" +
				   MatchRunner.Locus + "</a></td><td>" +
				   Temp1 + CellSep +
				   Temp2 + CellSep);
		    OutgoingTXT.write(
				      MatchRunner.Locus + tab
				      + Temp1 + tab
				      + Temp2 + tab
				      );
		    if (MatchRunner.SequenceLength == 0) {
			Outgoing.write("<a href=\"\">?</a></td><td><dfn>Not found in database</td></td></tr>\n"
				       );
			OutgoingTXT.write(
			...
			// Remove inappropriate characters for HTML from the description...
			Descrip.replace('&','*');
			Descrip.replace('<','*');
			Descrip.replace('>','*');
			Descrip.replace('\"','*');
			...
			Outgoing.write(MatchRunner.Coverage +
				       ...
				       "</a></td><td>" +
				       new Integer(MatchRunner.SequenceLength).toString() + /*
				       CellSep +
				       Protein.RoundTo(100f*MatchRunner.ProtConf,1) + "%" +
				       CellSep +
                                       Protein.RoundTo(100f*MatchRunner.ProtFP,1) + "%" + */
                                       CellSep +
				       new Integer(Math.round(MatchRunner.MolWt)).toString() +
				       CellSep +
				       new Float(Protein.RoundTo(MatchRunner.pI,1)).toString() +
				       CellSep);
			OutgoingTXT.write(
					  MatchRunner.SequenceCoverage + "%\t"
					  + new Integer(MatchRunner.SequenceLength).toString()
			...
			if (MatchRunner.AuxInfo != null) {
				...
				if (AIRunner.IsFloat) {
				    Outgoing.write(new Float(PoRunner.Intensity).toString());
				    OutgoingTXT.write(new Float(PoRunner.Intensity).toString());
				}
				else {
				    Outgoing.write(new Integer(Math.round(PoRunner.Intensity)).toString());
				    OutgoingTXT.write(new Integer(Math.round(PoRunner.Intensity)).toString());
				}
			...
			// Write the descriptive name of this locus to HTML and TXT output files
		...
		Outgoing.write("<table>\n");
                Outgoing.write("<tr><TH> <th>Filename<th>" +
                                   hXCorr + "<TH>" +
                                   hDeltCN + "<th>Conf%<th>ObsM+H+<th>" +
                                   hCalcPreMass + (Cutoffs.DisplayDeltaMass ? "<th>PPM" : "") + "<th>" + hSpR + "<th>" +
                                   hSpScore + 
		...
		while (DTARunner != null) {
			...
			OutgoingTXT.write(new Byte(DTARunner.EquivSeq).toString());
		    }
			...
			break;
                    ...
                    OutgoingTXT.write(tab +
                                      DTARunner.FileName + tab +
                                      DTARunner.XCorr + tab +
                                      DTARunner.DeltCN + tab +
				      Protein.RoundTo(100f*(1.0f-new Float(DTARunner.PepFP).floatValue()),1) + tab +	
                                      DTARunner.PrecursorMass + tab +
                                      DTARunner.CalcPreMass + tab +
                                      (Cutoffs.DisplayDeltaMass && Cutoffs.ExtraColumns ? Protein.RoundTo(new Float(DTARunner.Adjusted_PPM_Offset).floatValue(),1) + tab : "") +
                                      DTARunner.TotalIntensity + tab +
                                      DTARunner.Sp + tab +
                                      DTARunner.SpScore + tab +
				      (Cutoffs.DisplayPI && Cutoffs.ExtraColumns ? Protein.RoundTo(DTARunner.CalculatepI(),2) + tab : "") +
                                      (Cutoffs.DisplayKD && Cutoffs.ExtraColumns ? Protein.RoundTo(DTARunner.CalculateKyteDoolittle(),1) + tab : "") +
                                      (Cutoffs.DisplayBB && Cutoffs.ExtraColumns ? Protein.RoundTo(DTARunner.CalculateBullBreese(),1) + tab : "") +
                                      (Cutoffs.DisplayHPLC && Cutoffs.ExtraColumns ? Protein.RoundTo(DTARunner.CalculateHPLCpH34(),1) + tab : "") +
                                      Protein.RoundTo(100f*DTARunner.IonProportion,1) + tab +
                                      DTARunner.Redundancy + tab +
                                      DTARunner.Sequence +
				      (Cutoffs.DisplaySeqPos ? tab + DTARunner.SequencePosition : ""));
		    /* Print details for each spectrum */
		    SpectrumDirectory = new File(this.SourceDirectory, DTARunner.Subdirectory);
			...
			Outgoing.write(NewShow + DTARunner.ShowString() +
                                       CellSep + new Float(DTARunner.XCorr).toString() +
                                       CellSep + new Float(DTARunner.DeltCN).toString() +
				       CellSep + new Float(Protein.RoundTo(100f - 100f*DTARunner.PepFP,1)).toString() + "%" +
                                       CellSep + new Float(DTARunner.PrecursorMass).toString() +
                                       CellSep + new Float(DTARunner.CalcPreMass).toString() +
                                       (Cutoffs.DisplayDeltaMass ? CellSep + new Float(Protein.RoundTo(new Float(DTARunner.Adjusted_PPM_Offset).floatValue(),1)).toString() : "") +
                                       CellSep + new Integer(DTARunner.Sp).toString() +
                                       CellSep + new Float(DTARunner.SpScore).toString() +
				       (Cutoffs.DisplayPI ? CellSep + new Float(Protein.RoundTo(DTARunner.CalculatepI(),2)).toString() : "") +
                                       (Cutoffs.DisplayKD ? CellSep + new Float(Protein.RoundTo(DTARunner.CalculateKyteDoolittle(),1)).toString() : "") +
                                       (Cutoffs.DisplayBB ? CellSep + new Float(Protein.RoundTo(DTARunner.CalculateBullBreese(),1)).toString() : "") +
                                       (Cutoffs.DisplayHPLC ? CellSep + new Float(Protein.RoundTo(DTARunner.CalculateHPLCpH34(),1)).toString() : "") +
                                       CellSep + new Float(Protein.RoundTo(100f*DTARunner.IonProportion,1)).toString() +
                                       "%</td><td>" + new Integer(DTARunner.Redundancy).toString() +
                                       BLAST1 +
			...
			try {
			    SpectrumFile = new File(SpectrumDirectory, DTARunner.FileName).getCanonicalPath();
			}
			...
			Outgoing.write(DisplayIons +
				       SpectrumFile + ".dta" +
				       (DTARunner.Modified ? DTARunner.DisplayIonsString(SequestParams) : "") +
				       StaticModString +
				       DTARunner.TrimmedSequence() + "\">" +
				       DTARunner.FileName +
				       ShowOut +
				       ...
				       "</a></td><td>" + new Float(DTARunner.DeltCN).toString() + /*
				       CellSep + new Float(Protein.RoundTo(100f*DTARunner.PepConf,1)).toString() + "%" + */
				       CellSep + new Float(Protein.RoundTo(100f - 100f*DTARunner.PepFP,1)).toString() + "%" +
				       CellSep + new Float(DTARunner.PrecursorMass).toString() +
				       CellSep + new Float(DTARunner.CalcPreMass).toString() +
                                       (Cutoffs.DisplayDeltaMass ? CellSep + new Float(Protein.RoundTo(new Float(DTARunner.Adjusted_PPM_Offset).floatValue(),1)).toString() : "") +
                    ...
                    if (!Cutoffs.Brief) {
		        Descrip = LocusList.PrintLinksForPeptide(DTARunner.FileName);
		...
		//Create the file
		File              PeptideDB = new File(CurrentDirectory, DBPrefix + "-Prot2Pep.txt");
		...
		PDBBW.flush();
		PDBBW.close();
	    }	    /* Manipulate to get nonredundant peptide counts.  NOTE:
		     * THIS DESTROYS THE INTEGRITY OF THE PROTEIN LIST!!!
		     * This operation results in a single protein with all the
		     * peptides attached to it.
		     */
		...
		NRPeptideFPRate = 1.0f * NRDecoyPeptideCounter / NRForwardPeptideCounter;
	    }
            if (NRDecoySpectrumCounter == 0) {
                ...
                NRSpectrumFPRate = 1.0f * NRDecoySpectrumCounter / NRForwardSpectrumCounter;
            }
	    ...
	    Outgoing.write("<tr><td>Unfiltered</td><td>" +
			   new Integer(this.UnfilteredLocusCount).toString() + CellSep +
			   new Integer(this.UnfilteredOUTCount).toString() + CellSep +
			   new Integer(this.UnfilteredSpectrumCount).toString() + "</td></tr>\n");
/*	    Outgoing.write("<tr><td>Redundant</td><td>" +
			   new Integer(RProteinCounter).toString() + CellSep +
			   new Integer(PeptideCounter).toString() + CellSep +
			   new Integer(SpectrumCounter).toString() + "</td></tr>\n"); */
            Outgoing.write("<tr><td>Filtered</td><td>" +
                           new Integer(Counter).toString() + CellSep +
                           new Integer(NRPeptideCounter).toString() + CellSep +
                           new Integer(NRSpectrumCounter).toString() + "</td></tr>\n");
	    if (!Cutoffs.HideDecoy) {
	        Outgoing.write("<tr><td>Forward matches</td><td>" +
			   new Integer(ForwardCounter).toString() + CellSep +
			   new Integer(NRForwardPeptideCounter).toString() + CellSep +
			   new Integer(NRForwardSpectrumCounter).toString() + "</td></tr>\n");
                Outgoing.write("<tr><td>Decoy matches</td><td>" +
                           new Integer(DecoyCounter).toString() + CellSep +
                           new Integer(NRDecoyPeptideCounter).toString() + CellSep +
                           new Integer(NRDecoySpectrumCounter).toString() + "</td></tr>\n");
                Outgoing.write("<tr><td>Forward FDR</td><td>" +
			   new Float(Protein.RoundTo(100f*ProteinFPRate,2)).toString() + "%" + CellSep +
                           new Float(Protein.RoundTo(100f*NRPeptideFPRate,2)).toString() + "%" + CellSep +
                           new Float(Protein.RoundTo(100f*NRSpectrumFPRate,2)).toString() + "%" + "</td></tr>\n");
	    }
	    ...
	    Outgoing.write((this.Cutoffs.UseCustomPath ? this.Cutoffs.CustomPath : this.SourceDirectory) + newline);
	    ...
	    OutgoingTXT.write("Unfiltered\t" +
			      ...
			      new Integer(this.UnfilteredSpectrumCount).toString() + newline);
/*	    OutgoingTXT.write("Redundant\t" +
			      new Integer(RProteinCounter).toString() + tab +
			      new Integer(PeptideCounter).toString() + tab +
			      new Integer(SpectrumCounter).toString() + newline); */
            OutgoingTXT.write("Filtered\t" +
                              new Integer(Counter).toString() + tab +
                              new Integer(NRPeptideCounter).toString() + tab +
	    ...
	    if (!Cutoffs.HideDecoy) {
	        OutgoingTXT.write("Forward matches\t" +
                ...
                OutgoingTXT.write("Decoy matches\t" +
                              new Integer(DecoyCounter).toString() + tab +
                              new Integer(NRDecoyPeptideCounter).toString() + tab +
	    ...
	    OutgoingTXT.write("Unclassified\t" +
			      UnclassifiedNRCount + tab +
			      UnclassifiedRCount + newline);
	    OutgoingTXT.flush();
	    OutgoingTXT.close();
	    // If the user has selected database output,
	    if (Cutoffs.PrintDB) {
		Mega.PrintDBPeptides(CurrentDirectory, DBPrefix,
				     hXCorr, hCalcPreMass, hDeltCN, hSpScore, Cutoffs);
	    }
	    ...
	    if (MSBug) {
		System.out.println("*** An error occured because of one of these possibilities:");
		System.out.println("*** 1) your SEQUEST results have been moved to a new directory");
		System.out.println("*** 2) you need to use the --CGI option.");
		System.out.println("*** Some links in the DTASelect.html file are likely to be broken.");
	    }
	}
	catch (IOException failure) {
	    System.out.println("\tSomething went wrong while writing output files");
	    System.out.println(failure);
	}
    }

    /* Print an HTML web page which reports all sequence alignments
       within each protein. */
	...
	try {
		...
		if (PRunner.Classification != LastClass) {
		    ...
		    CurrentClassification = Classifieds.GetClass(PRunner.Classification);
		    if (CurrentClassification != null) {
			Outgoing.write("<h2>" +
				       CurrentClassification.Descriptor +
				       "</h2>\n");
		...
		while (DTARunner != null) {
		    ...
		    // Determine if the current peptide fits the current alignment
		    if (DTARunner.SequencePosition > RightMostResidue) {
			// We must start a new table row; this peptide isn't in the last contig.
			if (NonFirstRow) {
			...
			Outgoing.write("</td></tr><tr><td>" +
				       new Integer(DTARunner.SequencePosition + 1).toString() + "<BR>");
			...
			if (DTARunner.SequencePosition + SeqLength > RightMostResidue)
			    RightMostResidue = DTARunner.SequencePosition + SeqLength;
		    }
		    // Write the row title (leftmost residue position) to the text file
		    TOutgoing.write("P\t" + DTARunner.FileName + Tab);
		    // Write the appropriate number of spaces to each of the two files
			...
			for (Looper = 0; Looper < SeqLength; Looper++) {
				...
				TOutgoing.write(new Character(SeqString.charAt(Looper)).toString().toLowerCase());
				break;
			    }
			}
		...
		TOutgoing.write("S\t" +
				...
				"%\t" +
				new Integer(PRunner.SequenceLength).toString() + "\n");
		HTMLAlignment.append("</pre></td></tr></table>\n<table border><tr><td>\n" +
				     new Integer(MaxDepth).toString() +
				     "</td><td>Maximum Depth\n</td></tr><tr><td>" +
				     new Integer(PeptideLengthSum).toString() +
    ...
    /* Print an HTML web page which reports all modifications found
     * for all modified proteins. */
	...
	String                  NewShow = "<a target=\"Win1\" HREF=\"" +
	    Config.SQTDisplay + "?Dr=" +
	    (this.Cutoffs.UseCustomPath ? this.Cutoffs.CustomPath : this.SourceDirectory) + "&Da=";
	String                  ShowOut =  "<a TARGET=\"Win1\" href=\"" +
	    Config.DTAOUTDisplay + "?OutFile=";
	StaticModCGI.append(SequestParams.CGIString());
	...
	try {
	    ...
	    TOutgoing.write("M\tResidue Modified\tMass Difference\tSequence Alignment\tFilename\tXCorr\tModCount\tCopy Count\n");
	    TOutgoing.write("N\tResidue Modified\tMass Difference\tModification Position\tResidue Modified\n\n");
	    while (PRunner != null) {
		if (PRunner.Classification != LastClass) {
		    LastClass = PRunner.Classification;
		    CurrentClassification = Classifieds.GetClass(PRunner.Classification);
		...
		if (ModList != null) {
			...
			Outgoing.write(new Integer(ModRunner.ModPositionInProt + 1).toString() + "<BR>");
			// Write the mass difference found for this
			// modification type
			MassShift = new Float(SequestParams.DiffMods.getMassShiftFor(ModRunner.ModType)).toString();
			...
			LastResidue = ModRunner.Peptide.TrimmedSequence().charAt(LastPosition - LeftMostResidue);
			...
			while (ModRunner != null &&
			    ...
			    TOutgoing.write("M\t" + new Integer(LastPosition + 1).toString() + "\t" +
					    MassShift + "\t");
			    // Store the data needed for each column
			    // in our StringBuffer objects.
			    // First, align the sequence against the others.
			    ToAdd = ModRunner.Peptide.SequencePosition - LeftMostResidue;
			    for (Looper = 0; Looper < ToAdd; Looper++) {
				SequenceStack.append(' ');
				TOutgoing.write(' ');
			    }
			    TOutgoing.write(ModRunner.Peptide.TrimmedSequence() + "\t");
			    SequenceStack.append(ModRunner.Peptide.TrimmedSequence());
			    SequenceStack.append("\n");
			    // Next, write the XCorr, linking it to
			    // the out file viewer
			    CurrentSubdir = new File(SourceDir, ModRunner.Peptide.Subdirectory);
			    CanonicalName = new File(CurrentSubdir, ModRunner.Peptide.FileName).getCanonicalPath();
			    // We need a separate link for OUTs only if we're using that format
			    if (Cutoffs.DisplayType == 0) {
				XCorrStack.append(ShowOut + CanonicalName + ".out\">");
			    }
				...
				// incorporated.
				FileNameStack.append(NewShow + ModRunner.Peptide.ShowString() + "\n");
			    }
			    else {
				FileNameStack.append(DisplayIons + CanonicalName + ".dta" +
						     ModRunner.Peptide.DisplayIonsString(SequestParams) +
						     StaticModString + ModRunner.Peptide.TrimmedSequence() +
						     "\">");
				FileNameStack.append(ModRunner.Peptide.FileName);
			    ...
			    TOutgoing.write(ModRunner.Peptide.Redundancy + "\n");
			...
			TOutgoing.write("N\t" + new Integer(LastPosition + 1).toString() + "\t" +
					MassShift + "\t");
			...
			Outgoing.write(ModLine.toString() +
				       SequenceStack.toString() + "</pre></td><td><pre> \n" +
				       FileNameStack.toString() + "</pre></td><td><pre> \n" +
				       XCorrStack.toString() + "</pre></td><td><pre> \n" +
				       ModCountStack.toString() + "</pre></td><td><pre> \n" +
    ...
    /* Create a list of modification objects that correspond to this
     * protein's modifications.  Each nonzero position in each
     * identication's modification string should result in a
     * modification object.
     */
	...
	while (Runner != null) {
		...
		for (Looper = 0; Looper < SeqLength; Looper++) {
			...
			MRunner.ModPositionInProt = Runner.SequencePosition + Looper;
			MRunner.ModType = ModString.charAt(Looper);
			MRunner.Peptide = Runner;
			MRunner.ModCount = new Integer(SpectrumModCount).toString();
		    }
		}
	    }
	    Runner = Runner.Next;
	...
	try {
		...
		Outgoing.write(new Integer(Runner.Similarity(Runner)).toString());
    ...
    /* Create the DTASelect.txt database from all the loci and DTAFiles
     * read from .out files
     */
	...
	try {
	    ...
	    Outgoing.write((this.Cutoffs.UseCustomPath ? this.Cutoffs.CustomPath : CurrentDirectory.getCanonicalPath()) + NewLine);
	    ...
	    Outgoing.write("Source\t" + this.SQTGenerator + Tab + this.SQTGeneratorVersion + NewLine);
    ...
    /* If the DTASelect.txt file has already been written, don't
     * bother with the .out files.  Just read the big flat-file
     * database that already exists.
     */
    public void ReadFromFile(File DataFile)
	throws IOException {
	...
	// Read the command-line options at the first run of DTASelect on this set
	...
	while (WholeLine != null) {
		...
		if (LineBuffer.equals("L")) {
		    try {
			this.UnfilteredLocusCount++;
			// add previous protein to the list (if it's got any peptides)
			...
			System.out.println("This line in the DTASelect.txt is missing some information:");
			System.out.println(WholeLine);
			System.exit(0);
		    }
		}
		// if this is a line describing a DTAFile...
		else if (BuildingLegitProtein &&
			 LineBuffer.equals("D")) {
		    try {
			this.UnfilteredSpectrumCount++;
		...
		// For future versions: depending on the version, call 
			...
			String[] ParsedLine = TempDTA.FileName.split("\\.");
                        TempDTA.ChargeState = new Byte(ParsedLine[3]).byteValue();
			/* If this one qualifies, store it in the list of
			 * the current locus.
			 */
		    ...
		    this.SequestParams.addDiffMod(ModMass, ModSymbol, ModResidues,
						  ModPreLoss, ModFragLoss);
		}
		    ...
		    this.UnfilteredLocusCount = new Integer(Parser.nextToken()).intValue();
		    this.UnfilteredOUTCount = new Integer(Parser.nextToken()).intValue();
		    this.UnfilteredSpectrumCount = new Integer(Parser.nextToken()).intValue();
		}
	    }
	...
	this.SequestParams.ApplyStaticMods();
    }

    /* When naming the columns in the Contrast.html file, the first
     * part of the name comes from the sample's alias, and the second
     * part comes from the criteria set's alias.  */
	...
	try {
	    CurrentDTASelectTxt = new File(this.SourceDirectory, "DTASelect.txt");
	    this.ReadFromFile(CurrentDTASelectTxt);
	    if (this.SequestParams == null) {
		try {
		    //Read sequest.params file
		    this.SequestParams = ParamsFile.ReadFile(new File(this.SourceDirectory));
		}
		catch(IOException failure) {
    ...
    /* Caution!  This function is used by both Contrast and DTASelect.
     * This function applies spectrum-specific criteria, determines
     * the redundancy of individual spectra, and then picks out the
     * best spectrum from each redundant set.  Then the program can
     * determine which proteins to include and which to exclude.  */
    public void ApplyCriteria() {
	/* If the user has specified the -n option, no criteria will
	 * be applied to individual spectra.  Otherwise, cut out the
	 * spectra that don't pass the appropriate criteria.  */
	if (Cutoffs.UseCriteria)
	    LocusList.DumpUnqualifiedDTAs(Cutoffs);
	/* Determine how many copies of each spectrum remain after the
	 * spectrum filtering. */
	LocusList.CalculateRedundancyForList(Cutoffs.PurgeDuplicateSequences);
	if (Cutoffs.UseCriteria) {
	    /* Apply locus-specific criteria, first removing redundant
	     * copies of spectra if specified and then removing
	     * proteins with insufficient redundancy, uniqueness, or
	     * peptide variety. */
	...
	/* Now that we almost have our final list of DTAs and Loci,
	 * calculate the sequence coverage for each */
	LocusList.CalculateCoverageForList(false);
	if (Cutoffs.UseCriteria) {
            LocusList.DitchProteinsWithLowSequenceCoverage(this.Cutoffs);
        }
	/* To remove subset proteins, we must first group the
	   identicals.  Since Contrast determines data set similarity
	   after this step, we must ungroup when we're running that
	   rather than DTASelect. */
    ...
    /* This method assigns auxiliary information to each protein
     * listed in the "AuxInfo.txt" file.  */
	...
	try {
		...
		if (Parser.hasMoreTokens()) {
		    LineBuffer = Parser.nextToken();
		    if (LineBuffer.equals("float") ||
			LineBuffer.equals("int")) {
			// Create a new Classification for this line.
			...
			if (PRunner.Next != null) {
				...
				while (Parser.hasMoreTokens()) {
				    ...
				    PoRunner = PoRunner.Next;
				    PoRunner.Intensity = new Float(Parser.nextToken()).floatValue();
				}
			    }
			    catch (NoSuchElementException failure) {
				System.out.println("\t\t" + LineBuffer +
    ...
    /*
     * If the user has specified that a DataSet is the master, all
     * others are compared to the master set and any protein not found
     * in the master set is removed.
     *
    ...
    /* This function is really the heart of Contrast.  A LocusSummary
     * is an object representing a Locus across multiple DataSets.
     * It's like a meta-Protein; the difference is that it stores
     * where it can be found in the FoundInPattern field.  The
     * FoundInPattern field is not just an integer; it's a pattern of
     * bits.  Each bit represents whether that Locus can be found in a
     * particular DataSet.  Which bit is affected is passed in as
     * "CurrentPower."  A zero means absence, and a one means presence.
     */
	...
	while (Cursor != null) {
	    /* If the next Locus in this DataSet falls after
	     * the next in the LS list, advance the LS list
	     */
	    ...
	    /* If the next Locus in this DataSet precedes the
	     * next in the LS list and doesn't yet appear in our
	     * LS list, copy it into the LS list.
	     */
	    ...
	    /* If the next Locus in this DataSet is the same
	     * name as the next in the LS list, update its
	     * FoundInPattern
	     */
	    else {
    ...
    /* Read the earliest locus from this collection of DataSets.  Sort
     * together peptides for this locus from each dataset, and return
     * the StringBuffer Contrast should put into the merged
     * DTASelect.txt
     */
	...
	while (DSRunner != null) {
	    /* WholeLine will be the first line in this DTASelect that
	     * followed the last read "D" line.  It should be either a
	     * "C" [count] line or a "L" [locus] line.  If WholeLine
	     * is null, this is an older class of DTASelect.txt that
	     * included no counts. */
	...
	if (!LociRemain) {
		...
		if ( (DSRunner.LocusCursor != null) &&
			...
			System.out.println("Could not keep reading from DTASelect.txt in " +
    ...
    /* What is the composite (sample and criteria set) name for the
     * specified DataSet?
     */
     ...
     * Contrast.html file.  Create the header for the specified
     * DataSet.
     */
    ...
    /* Pass a Locus name to the list of DataSets.  In each DataSet,
     * set its Cursor to point at the Protein matching that locus
     * name.  If the locus doesn't exist, point to null.
     */
    ...
    /* Return a pointer to the Protein object pointed to by the
     * specified DataSet's cursor.  Send back a null if the cursor is
    ...
    /* Set the sequence coverage percentage and consensus fields for
     * the specified protein in the specified data set. */
    public void SetLocusCoverage(int WhichSet, String Consensus, float
				 PercentCoverage) {
	DataSet      Runner = this.Next;
	for (int Looper = 0; Looper < WhichSet; Looper++) {
	    Runner = Runner.Next;
	...
	if (Runner.LocusCursor == null) {
	    System.out.println("Attempted to set coverage for a nonexistent locus");
	}
	else {
    ...
    /* This function is called when the protein's sequence is null (of
     * length 0).  Sequence coverage is undefined in this case. */
    ...
    /* If Contrast's verbose mode is in use, determine the XCorr for
     * the first peptide in the specified DataSet's current protein
     * that matches the passed TargetSequence.  This allows a
     * comparison of XCorr scores for the same peptide across multiple
     * samples.
     */
    ...
    /* Determine the current Protein's sequence coverage percentage
	...
	public void DebugPrint() {
	    ...
	    while (Runner != null) {
		System.out.println(new Integer(Runner.ModPositionInProt).toString() + "\t" +
