...
public class LDAConfidence {
    ...
    boolean		UseXCorr, UseDeltCN, UseSp, UseMassDifference;
	...
	DataPoints = new double[NDim][NTotal];
        ...
        Runner = LocusList;
        NDirect = 0; NReverse = 0; counter = 0; 
        while (Runner.Next != null) {
            Runner = Runner.Next;
    ...
    public LDAConfidence(OUTFile OutFileList, SelectCriteria Cutoffs, int Charge, boolean Modified, byte Tryptic, boolean Status) {
	...
	FlagName = (UseChargeState ? new Integer(ChargeState).toString() : "all") + "." +
	...
	if (NDim != 0) {
	    ...
	    System.out.println("No statistics criteria selected! Use --nostats if you want that. Exiting...");
	    System.exit(0);
	}

	// Initialize Positive array
	OUTRunner = OutFileList;
	counter = 0; NValidated = 0; NDirect = 0; NReverse = 0;
	while (OUTRunner.OriginalNext != null) {
	    OUTRunner = OUTRunner.OriginalNext;
	...
	dim_counter = 0;
	    ...
	    DeltaMassIndex = dim_counter;
            ...
            while (OUTRunner.OriginalNext != null) {
                ...
                if (OUTRunner.OngoingStatisticsFlag) {
                    if (UseLogMass)
                        DataPoints[dim_counter][counter] = - Math.log(Math.max(Math.abs(OUTRunner.DTA.Adjusted_PPM_Offset),0.000001));
                    else {
                        DataPoints[dim_counter][counter] = - Math.abs(OUTRunner.DTA.Adjusted_PPM_Offset);
		...
		System.out.println("\tMass Deviation is:\t" + AverageMassDeviation + " PPM");
	        for (counter = 0; counter < NTotal; counter++) {
		    DeltaMass[counter] = DeltaMass[counter] - AverageMassDeviation;
		    DataPoints[dim_counter][counter] = -Math.abs(DeltaMass[counter]);
	        }
	    }
            dim_counter++;
        }
	...
	if (UseXCorr) {
	    ParameterName[dim_counter] = (UseRawXCorr ? "XCorr" : "XCorrNorm");
			...
			DataPoints[dim_counter][counter] = Math.log(Math.max(XCorr, 0.1))/LogNL;
		    }
		    else {
	    ...
	    ParameterName[dim_counter] = "Sp";
            ...
            while (OUTRunner.OriginalNext != null) {
                OUTRunner = OUTRunner.OriginalNext;
                if (OUTRunner.OngoingStatisticsFlag) {
		    DataPoints[dim_counter][counter] = new Double(OUTRunner.DTA.SpScore).doubleValue();
                    counter++;
                }
            }
            dim_counter++;
        ...
        if (UseLogSpRank) {
            ...
            while (OUTRunner.OriginalNext != null) {
                OUTRunner = OUTRunner.OriginalNext;
                if (OUTRunner.OngoingStatisticsFlag) {
                    DataPoints[dim_counter][counter] = - Math.log(new Double(OUTRunner.DTA.Sp).doubleValue());
                    counter++;
                }
            }
            dim_counter++;
        ...
        if (UseIonStat) {
            ...
            while (OUTRunner.OriginalNext != null) {
                OUTRunner = OUTRunner.OriginalNext;
                if (OUTRunner.OngoingStatisticsFlag) {
                    DataPoints[dim_counter][counter] = new Double(OUTRunner.DTA.IonProportion).doubleValue();
                    counter++;
                }
            }
            dim_counter++;
	...
	String FileName = "";
        ...
        while (OUTRunner.OriginalNext != null) {
            ...
            if (OUTRunner.OngoingStatisticsFlag) {
                ...
                counter++;
		OUTRunner.OngoingStatisticsFlag = false;
		OUTRunner.DTA.PepFP = new Float(1.0 - Integral[counter]).floatValue();
		OUTRunner.DTA.PepConf = new Float(Confidence[counter]).floatValue();
    ...
    public void PlotDirectReverse(String FileName, int dim1, int dim2, boolean positive) {
        File                    OutputFile;
	...
	System.out.println("Writing plotting information to " + FileName + "...");
		...
		if (Positive[counter] == positive)
                    Outgoing.write((DeltaMassIndex == dim1 ? new Double(DeltaMass[counter]).toString() : new Double(DataPoints[dim1][counter]).toString()) + "\t" + (DeltaMassIndex == dim2 ? new Double(DeltaMass[counter]).toString() : new Double(DataPoints[dim2][counter]).toString()) + "\n");
            }
            Outgoing.flush();
            Outgoing.close();
        }
...
+ ".");
	...
	int counter;
        ...
        try {
            ...
            Outgoing = new BufferedWriter(OutputFileWriter);
            for (counter = 0; counter < NTotal; counter++) {
                Outgoing.write(new Double(MDistance[counter]).toString() + "\t" + new Double(Probability[Index[counter]]).toString() + "\n");
            }
            Outgoing.flush();
            Outgoing.close();
        }
...
+ ".");
		...
		if (Positive[Index[counter]]) {
                Outgoing.write(new Double(MDistance[counter]).toString() + "\t" + new Double(Integral[Index[counter]]).toString() + "\n");
                }
	    }
            Outgoing.flush();
            Outgoing.close();
...
+ ".");
	...
	System.out.println("####################################################");
        this.PrintTotals();
	for (counter = 0; counter < NTotal; counter++) {
	    System.out.println("RealDB " + 
				Positive[counter] + "\tConfidence " + Confidence[counter]);
	}
	System.out.println("----------------------------------------------------");
    }
    ...
    public void DebugPrintMD() {
        ...
        for (counter = 0; counter < NTotal; counter++) {
            System.out.println("RealDB " +
                                Positive[counter] + "\tMDistance " + MDistance[counter]);
        }
        System.out.println("----------------------------------------------------");
    }
	...
	for (counter = NTotal-1; counter > -1; counter--) {
	    System.out.println(counter + "\t" + CompositeScore[counter] + "\t" + Positive[Index[counter]]);
	}
	...
	if (NDirect == 0 || NReverse == 0) {
	    System.out.println("Not enough data for statistics on this set! Skipping...");
	    return;
	}
	if (PlotLevel >= 1) {
	    for (dim1 = 0; dim1 < NDim; dim1++) {
		for (dim2 = dim1 + 1; dim2 < NDim; dim2++) {
		    PlotDirectReverse(ParameterName[dim1] + "." + ParameterName[dim2] + ".Direct." + FlagName + ".dat", dim1, dim2, true);
		    PlotDirectReverse(ParameterName[dim1] + "." + ParameterName[dim2] + ".Reverse." + FlagName + ".dat", dim1, dim2, false);
		}
	    }
	}
//	PrintClassification();
	for (dim_counter = 0; dim_counter < NDim; dim_counter++) {
	    ...
	    SingleScore = new SingleDimensionScore(ParameterName[dim_counter] + "." + FlagName, NTotal, BufferScore, Positive);
	    if (PlotLevel >= 2) {
		SingleScore.GetROC();
...
//	    PrintClassification();
	...
	if (NDim > 1) {
	    if (CalculateCoeffOnly) {
	        if (UseTrueProtein)
		    determinant = NRUtils.LDA(DataPoints, MDistance, LinearCoefficients, NDim, NTotal, Validated);
	        else
	            determinant = NRUtils.LDA(DataPoints, MDistance, LinearCoefficients, NDim, NTotal, Classification);
	        if (determinant == 0.0) {
                    System.out.println("Determinant is zero in inverse matrix calculation!");
                    System.out.println("Skipping linear coefficients calculation!");
                    return; 
                }
            }
	    else {
		...
		for (counter = 0; counter < NTotal; counter++) {
            	    ...
            	    for (dim_counter = 0; dim_counter < NDim; dim_counter++) {
                	MDistance[counter] += DataPoints[dim_counter][counter] * LinearCoefficients[dim_counter];
            	    }
        	}
	    }
	    SingleScore = new SingleDimensionScore("Mahalanobis." + FlagName, NTotal, MDistance, Positive);
	}
	else {
	    ...
	    SingleScore = new SingleDimensionScore(ParameterName[0] + "." + FlagName, NTotal, MDistance, Positive);
	}
	if (PlotLevel >= 2) {
	    SingleScore.GetROC();
	    SingleScore.GetCutoffs(1.0 - FPTol, TrueProt);
