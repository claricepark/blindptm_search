...
public class Protein {
     ...
     * These are functions to be called on an individual Protein in a
     * list rather than on the list header itself
     */
    ...
    /* Set the fields of this protein to the contents of this
     * StringTokenizer (which delimits by tabs and has the initial "L"
...
//	    this.ProtFP = new Float(Parser.nextToken()).floatValue();
    ...
    /* Compare this protein to the passed one and produce a measure of
     * its similarity.

     * (9/26/02) I noticed that this code relied upon the peptides
     * being ordered alphabetically by sequence.  Now this code
     * presumes nothing about the order.
     */
    ...
    /* Return the salient features of this Protein, separating with tabs.
       Used for GUI output
    */
...
//             new Float(RoundTo(ProtFP,2)).toString() + "\t" +
	       ...
	       new Float(RoundTo(pI,1)).toString() + "\t"+
	       Gene);
    }

    /* Return the "L" line for DTASelect.txt corresponding to this
     * protein.  Used both by DataSet.PrintTXT and
     * DataSet.PullTopLocus */
    public String GetLLine() {
	String Tab = "\t";
	return ("L\t" + this.Locus + Tab +
		new Integer(this.SequenceLength).toString() + Tab +
//		new Float(this.ProtConf).toString() + Tab +
//		new Float(this.ProtFP).toString() + Tab +
		new Float(this.MolWt).toString() + Tab +
		new Float(this.pI).toString() + Tab +
		this.Gene + Tab +
		this.Validated + "\n");
    }

    /* For Contrast verbose mode, be able to create a Protein object
     * that includes all peptides from Proteins found in each
     * combination of sample and criteria set.  This function is used
     * to accumulate a master Protein by including all the peptides of
     * Proteins passed to it.  It's just a straight cosequential
     * algorithm like one would use in a merge sort.  */
	...
	if (UniListing.length == 0) {
		...
		for (FileIterator = 0; FileIterator < DirectoryListing.length; FileIterator++) {
		    CurrentSQT = new File(TargetDir, DirectoryListing[FileIterator]);
		    CurrentSQT.delete();
		}
	    }
	    else {
	    ...
	    /* Visit each SQT file.  Read it into memory, sort it, and
	       compare it to the list of IDs passing filters to
	       determine which are kept. */
		...
		System.out.println("\t\tReading " + CurrentBaseName + " into memory...");
		CurrentSQT = new File(CurrentDirectory, CurrentBaseName);
		RealCount = 0;
		SQTCount = 0;
		TargetFile = new File(TargetDir, CurrentBaseName);
		CurrentBaseName  = CurrentBaseName.substring(0, CurrentBaseName.length() - 4);
		try {
			...
			try {
			    ...
			    // Skip the S
			    Parser.nextToken();
			    TBRunner.Index = new Integer(Parser.nextToken()).intValue() * 10;
			    // Skip the second scan
			    Parser.nextToken();
			    TBRunner.Index += new Integer(Parser.nextToken()).intValue();
			    WholeLine = Incoming.readLine();
			}
			catch (NumberFormatException failure) {
			    System.out.println("This line of " + CurrentBaseName +
					       ".sqt was garbled:");
			...
			// Mark each in the unified file which is retained in memory
			...
			while ( (DTARunner != null) &&
			    ...
			    SoughtIndex = new Integer(Parser.nextToken()).intValue() * 10;
			    // skip hiscan
			    Parser.nextToken();
			    // use charge state
			    SoughtIndex += new Integer(Parser.nextToken()).intValue();
			    TBRunner = Contents.Next;
			    while ( (TBRunner != null) &&
				...
				if (TBRunner.Index == SoughtIndex) {
				    ...
				    System.out.println("SQT: SoughtIndex " + SoughtIndex + " is different from TBRunner.Index " + TBRunner.Index);
				}
			    }
			    else {
				    System.out.println(DTARunner.FileName + " could not be found in the sqt file.");
		    	    }
			    DTARunner = DTARunner.Next;
			}
			// Write new copy of unified file
			System.out.println("\t\tWriting " + SQTCount + " / " + RealCount + " IDs to new file...");
			Report.append(CurrentBaseName + "\t" + SQTCount + "\t" + RealCount + newline);
			try {
			    ...
			    System.out.println("An IO Error occured while writing " + TargetFile.toString());
			    System.out.println(failure);
			}
		    }
		}
		catch (IOException failure) {
		    System.out.println("IO Error while reading " + CurrentBaseName);
		    System.out.println(failure);
		}
	    }
	...
	StringBuffer     Header;
	StringBuffer     CurrentSection;
	StringBuffer     Report = new StringBuffer("MS2File\tSpectraKept\tSpectraFound\n");
	String           newline = "\n";
	TextBlock        Contents;
	TextBlock        TBRunner;
	DTAFile          DTARunner = DTAs.Next;
	...
	if (UniListing.length == 0) {
		...
		DirectoryListing = TargetDir.list(FilenameFilter);
		for (FileIterator = 0; FileIterator < DirectoryListing.length; FileIterator++) {
		    CurrentMS2 = new File(TargetDir, DirectoryListing[FileIterator]);
		    CurrentMS2.delete();
		}
	    }
	    else {
	    ...
	    /* Visit each MS2 file.  Read it into memory, sort it, and
	       compare it to the list of IDs passing filters to
	       determine which are kept. */
		...
		try {
			...
			DTARunner = DTARunner.Next;
		    if (DTARunner == null) {
			System.out.println("\t\tNo retained spectra for this file");
		    }
		    else {
			// Mark each in the unified file which is retained in memory
			TBRunner = Contents.Next;
			while ( (DTARunner != null) &&
				...
				if (TBRunner.Index == SoughtIndex) {
				    ...
				    System.out.println("MS2: SoughtIndex " + SoughtIndex + " is different from TBRunner.Index " + TBRunner.Index);
				}
			    }
			    else {
				    System.out.println(DTARunner.FileName + " could not be found in the ms2 file.");
			    }
			    DTARunner = DTARunner.Next;
			}
			// Write new copy of unified file
			System.out.println("\t\tWriting " + MS2Count + " / " + RealCount + " spectra to new file...");
			Report.append(CurrentBaseName + "\t" + MS2Count + "\t" + RealCount + newline);
			try {
			    OutputFileWriter = new FileWriter(TargetFile);
			    Outgoing = new BufferedWriter(OutputFileWriter);
			    Outgoing.write(Header.toString());
    ...
    public void PrintDBPeptides(File CurrentDirectory, String DBPrefix,
				String hXCorr, String hCalcPreMass,
				String hDeltCN, String hSpScore, SelectCriteria Cutoffs) {
	...
	try {
	    ...
	    PDBBW.write("MatchID\tZ\t" + hXCorr + "\t" +
			hDeltCN + "\tM+H+\t" + hCalcPreMass + (Cutoffs.DisplayDeltaMass && Cutoffs.ExtraColumns ? "\tPPM" : "") +
			"\tSpR\t" + hSpScore + (Cutoffs.DisplayPI && Cutoffs.ExtraColumns ? "\tpI" : "") +
		...
		PDBBW.write(
			...
			(Cutoffs.DisplayDeltaMass && Cutoffs.ExtraColumns ? "\t" + Protein.RoundTo(new Float(DTARunner.Adjusted_PPM_Offset).floatValue(),1) : "") +
			"\t" + new Integer(DTARunner.Sp).toString() +
			"\t" + new Float(DTARunner.SpScore).toString() +
			(Cutoffs.DisplayPI && Cutoffs.ExtraColumns ? "\t" + Protein.RoundTo(DTARunner.CalculatepI(),2) : "") +
			(Cutoffs.DisplayKD && Cutoffs.ExtraColumns ? "\t" + Protein.RoundTo(DTARunner.CalculateKyteDoolittle(),1) : "") +
                        (Cutoffs.DisplayBB && Cutoffs.ExtraColumns ? "\t" + Protein.RoundTo(DTARunner.CalculateBullBreese(),1) : "") +
                        (Cutoffs.DisplayHPLC && Cutoffs.ExtraColumns ? "\t" + Protein.RoundTo(DTARunner.CalculateHPLCpH34(),1) : "") +
	    ...
	    System.out.println("Error while writing " + DBPrefix + "-Peptides.txt");
	}
    }
    
    /* Return list of peptides for this locus.  Used for Contrast's
     * verbose mode during output. */
    ...
    /* Determine whether this Protein has at least one unique peptide
     * associated with it.  This corresponds to the -u criterion. */
    ...
    /* Determine how many times each sequence shows up among the
     * peptides for this locus.  Store this value with each peptide.
     * This must be generated each time DTASelect is run since the
     * criteria may eliminate some duplicate spectra and this number
     * is generated after the spectrum-specific criteria are
     * imposed. */
    public void CalculateRedundancy(int Purge) {
	DTAFile        Runner = this.DTAs.Next;
	DTAFile        SubRunner;
	short          Count;
	...
	while (Runner != null) {
	    if (Runner.Tryptic >= Cutoffs.MinProtTryptic && Runner.SpScore >= Cutoffs.MinProtSpScore && Runner.XCorr >= Cutoffs.MinProtXCorr && 
                Runner.PepFP <= Cutoffs.MaxProtMinFP && Runner.PepConf >= Cutoffs.MinProtPepConf && Math.abs(Runner.Adjusted_PPM_Offset) <= Cutoffs.MaxProtDM) {
	        return true;
	    }
	    Runner = Runner.Next;
	}
    ...
    /* GenerateZones helps determine how many residues of the locus
     * sequence are represented by residues in the DTAFiles.  A
     * CoverageZone object is created for each peptide remaining for
     * this protein. */
    public CoverageZone GenerateZones() {
	DTAFile        Runner = this.DTAs.Next;
	CoverageZone   Zones = new CoverageZone();
	CoverageZone   ZRunner = Zones;
	/* Create a list of CoverageZones that corresponds to all
	 * DTAFiles that may map to this locus.  Each CZ stores the
	 * first and last string indecies covered by an individual
	 * peptide sequence.
	 */
	while (Runner != null) {
    ...
    /* Determine where each DTAFile sequence maps to the full protein
     * sequence.  Since SEQUEST incorrectly implies that DTAFiles with
     * multiple locus matches have the same sequence context, fix the
     * sequence contexts for all peptides (by changing the first and
     * last characters of the Sequence field).
     * If a subsequence doesn't match the database sequence to which
     * it is mapped, send a message to the user and prepare to panic.
     */
    public void FindDTAPositions(String Sequence, String Locus, SelectCriteria Cutoffs) {
	DTAFile     Runner = this.DTAs.Next;
	if (Sequence.length() > 0) {
		...
		if (Subsequence.length() == 0) {
			...
			if (Runner.SequencePosition == 0)
			    ...
			    Leader = Sequence.charAt(Runner.SequencePosition - 1);
			if (Runner.SequencePosition + Subsequence.length() == Sequence.length())
			    Trailer = '-';
			else
			    Trailer = Sequence.charAt(Runner.SequencePosition + Subsequence.length());
			Runner.Sequence = Leader + "." +
			    Runner.Sequence.substring(2,Runner.Sequence.length()-2) + "." + Trailer;
			byte TempTryptic = Runner.Tryptic;
			Runner.DetermineTryptic(Cutoffs);
/*			if (Runner.Tryptic != TempTryptic)
			    System.out.println("Warning: tryptic status is actually " + Runner.Tryptic + " rather than " + TempTryptic + " for " + Runner.FileName + "!"); */
		    }
		}
		Runner = Runner.Next;
	    }
     ...
     * These functions are those called on the header node of the
     * Protein list.  They always loop through the entire linked list
     * of Proteins, performing some operation on each.
     */

    /* Create one protein with all the peptides from all proteins in
     * this list.  This is useful for getting a nonredundant count of
     * peptides and proteins.
     * NOTE!  This destroys the current list of proteins!!!  Do not
     * use if further printing is planned!  */
	...
	while (DTARunner != null) {
	    while (DTARunner.Next != null && DTARunner.Next.FileName.equals(DTARunner.FileName) ) {
	...
	try {
	    ...
	    SumTotals[3] = 0;
	    while (DatStart != null) {
		/* Assume DatStart points at the first spectrum ID of
		 * a set created from a single .dat file. */
		...
		while ( (DatFinish != null) &&
			(Buffer.equals(CurrentPrefix)) ) {
		...
		/*
		InsideRunner = DatStart;
		LastBin = 0;
		LastCount = 0;
		while (InsideRunner != DatFinish) {
		    Parser = new StringTokenizer(InsideRunner.FileName,".");
		    CurrentPrefix = Parser.nextToken();
		    CurrentPrefix = Parser.nextToken();
		    CurrentBin = new Integer(CurrentPrefix).intValue() / BinSize;
		    while (LastBin != CurrentBin) {
    ...
    /* For each protein in the list, examine the other proteins to
     * determine whether their peptides are supersets of the current
     * protein's peptides.  If so, remove this protein.  NOTE: Run
     * after proteins have been grouped for identical sets of
     * peptides.
     * Note that this function compares peptides by sequence and
     * charge state only.  If peptides have different sequence context
     * information, they're considered different peptides even if they
	...
	while ( (ThisRunner != null) && (ThisRunner.Next != null) ) {
		...
		if (OtherRunner != ThisRunner.Next) {
		    ...
		    if (IsSuperset) {
			System.out.println("\t" + ThisRunner.Next.Locus + "\t" + OtherRunner.Locus);
			RemovedTotal++;
		    }
		}
		OtherRunner = OtherRunner.Next;
	...
	System.out.println("\tRemoved " + new Integer(RemovedTotal).toString() + " subset proteins.");
    }

    /* Grab every protein and peptide from Other list and add it to
     * this list.  Use a basic cosequential algorithm. */
	...
	while ( (ThisRunner.Next != null) && (OtherRunner != null) ) {
		...
		/* Other's current protein falls after this.Next.
		 * Advance to next in this list and repeat
		 * comparison. */
		ThisRunner = ThisRunner.Next;
	    }
	    else if (ThisRunner.Next.Locus.compareTo(OtherRunner.Locus) == 0) {
		/* Other's current protein is the same as this.Next.
		 * Append in the peptides in sorted order.  */
		ThisRunner.Next.AccumulateDTAsFrom(OtherRunner);
		OtherRunner = OtherRunner.Next;
	    }
	}
	/* If there are still proteins to be found in the Other list,
	 * append those to the end of this list. */
	while (OtherRunner != null) {
	    ThisRunner.Next = OtherRunner.Clone();
	    ThisRunner = ThisRunner.Next;
	    OtherRunner = OtherRunner.Next;
    ...
    /* Return the longest protein name in this list.
     * Used for DTASelectGUI. */
    ...
    /* Return the longest DTA file name in this list.  Used for
     * DTASelectGUI. */
    ...
    /* Return the first Protein object in the list matching the
     * supplied locus name.  Used for DTASelectGUI. */
    ...
    /* Return the first DTA file matching the given sequence.
       Used for DTASelectGUI.
    */
    ...
    /* Link each protein to all others remaining with identical
     * peptides.
     */
    ...
    /* Link each protein to all others remaining with identical
     * sequence coverage.  One must run "CalculateCoverageForList()"
     * before this is run.
     */
    ...
    /* Undo what the above function does.  Put all the Proteins
     * attached at IdenticalLoci pointers back into their proper
     * places in the list.  */
	...
	while (SSRunner != null) {
	    ...
	    while ( (Runner.Next != null) && (Runner.Next.Locus.compareTo(SSRunner.Locus) < 0) )
		Runner = Runner.Next;
    ...
    public void CalculateRedundancyForList(int Purge) {
	Protein         Runner = this.Next;
	while (Runner != null) {
	    Runner.CalculateRedundancy(Purge);
	    Runner = Runner.Next;
	}
    }

    /* Print a series of HTML links to other occurrences of this
     * peptide within the list
     */
	...
	StringBuffer        Returned = new StringBuffer("");
        ...
        String              Trunc = ParsedLine[0] + "." + ParsedLine[1] + "." + ParsedLine[2];
	while (PRunner != null) {
	    DRunner = PRunner.DTAs.Next;
	    while ( (DRunner != null) &&
		    (!DRunner.FileName.startsWith(Trunc)) )
		DRunner = DRunner.Next;
	    if (DRunner != null) {
    ...
    /* Determine sequence coverage for each Protein in the list.
     * Store both the percentage and the list of peptides (after
	...
	while (OuterRunner != null) {
		...
		/* Sum together the remaining CoverageZones' spans and divide
		 * by the full protein length to determine percentage of
		 * coverage.
		 */
		BRunner = Buffer.Next;
		while (BRunner != null) {
		...
		Runner.SequenceCoverage = Protein.RoundTo((100.0f *
							   SCAccumulator /
							   Runner.SequenceLength),1);
    ...
    /* Open the FASTA database indicated in sequest.params.  Locate
     * each identified locus in that file, and store the sequence
     * found with each Protein object.  In addition, store the
     * descriptive name for each locus in the Protein object.  FASTA
     * databases are not necessarily sorted by locus! */
    public void LookUpLoci(ParamsFile SEQUESTParams, IniFile Config, SelectCriteria Cutoffs) throws IOException {
	...
	if (this.Next == null) {
	    ...
	    System.out.println("If .out files are in the current directory, use the --here or -. options");
	    System.exit(0);
	}
	else {
		...
		if (!Parser.hasMoreTokens()) {
		     ...
		     * names. Binary searches are even shorter names!*/
			...
			Runner = Runner.Next;
		    if ((Runner != null) && (LineBuffer.startsWith(Runner.Locus))) {
			/* This DB locus corresponds to the current
			 * identified locus. Read gene name and
			 * sequence */
			...
			Runner.CChar = Sequence.charAt(Runner.SequenceLength - 1);
    ...
    /* Run through the entire Protein list.  Run through each DTAFile
     * for each Protein.  Eliminate DTAFiles that don't meet the
    ...
    /* Run through the list of Proteins.  Count how many DTAFiles are
     * associated with each locus.  Remove Proteins with insufficient
     * numbers of supporting DTAFiles.
     */
    ...
    /* Run through the list of Proteins.  Check DTAFiles for duplicate
     * sequences.  Destroy all but one of the DTAFiles for any given
     * sequence, retaining the one with the highest XCorr.
     */
    ...
    /* Run through the list of Proteins.  Check DTAFiles for duplicate
     * sequences.  Destroy all but one of the DTAFiles for any given
     * sequence, retaining the one with the highest TIC.
     */
	...
	Protein    PRunner = this.Next;
	while (PRunner != null) {
	    System.out.println(PRunner.Locus + "\t" +
			       PRunner.Classification + "\t" +
			       PRunner.MolWt);
	    PRunner = PRunner.Next;
	}
    }
    ...
    /* Run through the list of Proteins.  Sort the DTAFiles for each
     * Protein on the basis of sequence.  Use the nifty quicksorter
     * in DTAFile for this process.
     */
    ...
    /* Print the list of DTAFile filenames as a series of cp commands.
     * We don't care which directory the files come from.  Nor do we
     * bother to write these to a file but instead splat them out to
     * the screen.  We're lazy.  That's why.

     * (9/26/02) Well, clearly I cleaned it up later on.  Now it
     * writes a platform-specific script to do the copying.  Progress!
     */
	...
	try {
		...
		if (Config.ServerType.equals("UNIX")) {
		    ...
		    while (DTAFileRunner != null) {
			Outgoing.write("copy " + DTAFileRunner.CanonicalName(CurrentDir) + " c:\\temp\\\n");
			Outgoing.write("copy " + DTAFileRunner.DTACanonicalName(CurrentDir) + " c:\\temp\\\n");
			DTAFileRunner = DTAFileRunner.Next;
		    }
		}
		Runner = Runner.Next;
    ...
    public void PrintDatabase(SelectCriteria Cutoffs, ParamsFile Params, String RootName) {
	Protein                 Runner = this.Next;
	Protein                 MatchRunner;
	...
	try {
		...
		while (MatchRunner != null) {
		    Outgoing.write(
				   ...
				   MatchRunner.NPeptides + tab +
				   MatchRunner.NSpectra + tab +
				   MatchRunner.SequenceCoverage + tab +
				   new Integer(MatchRunner.SequenceLength).toString() + tab +
				   new Integer(Math.round(MatchRunner.MolWt)).toString() + tab +
				   new Float(RoundTo(MatchRunner.pI,1)).toString() + tab +
				   MatchRunner.Gene);
	...
	try {
	    ...
	    Outgoing.write("\n<DTASelect:Header>\n");
	    Outgoing.write("\t<DTASelect:Version>" + Protein.Version() + "</DTASelect:Version>\n");
	    Outgoing.write("\t<DTASelect:Directory>" + DirName + "</DTASelect:Directory>\n");
	    Outgoing.write("\t<DTASelect:Database>" + Params.DBName + "</DTASelect:Database>\n");
	    Outgoing.write("\t<DTASelect:CommandLine>" + CommandLineOptions + "</DTASelect:CommandLine>\n");
		...
		while (MatchRunner != null) {
			...
			Outgoing.write(
					   ...
					   + "</DTASelect:SeqLength>\n"
				       + "\t\t<DTASelect:ProtConf>"
				       + new Float(RoundTo(MatchRunner.ProtConf,2)).toString()
				       + "</DTASelect:ProtConf>\n"
                                       + "\t\t<DTASelect:ProtFP>"
                                       + new Float(RoundTo(MatchRunner.ProtFP,2)).toString()
                                       + "</DTASelect:ProtFP>\n"
				       + "\t\t<DTASelect:MW>"
