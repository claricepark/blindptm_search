...
//    private Modifications mods = new Modifications(); 
    ...
    private void addModifications(Modifications m) {
        ...
        if(m != null && m.getDiffModsShift() != 0 && !allModifications.contains(m) && m.getNumDiffMods()<=maxAlter) {
...
//System.out.println("in readTerminalMods, ctermStaticMod: " + ctermStaticMod + "\tntermStaticMod: " + ntermStaticMod);
            ...
            if(diffMods != null) {
                ...
                for(Element de : diffModList) {
                    ...
                    String symbol = de.getChildTextTrim("symbol");
                    ...
                    if (massShift != null && !"".equals(massShift.trim())) {
                        for(char c: symbol.toCharArray())
                        {
                            ...
                            if(mass != 0) {
                                TerminalModification d = new TerminalModification(c, mass);
                                if(isCterm) {
                                    ctermDiffMods.add(d);
                                } else {
                                    ntermDiffMods.add(d);
    ...
    private void readModifications(Element me) {
        ...
        Element compoundElement = me.getChild("compound");
        if(compoundElement!=null)
        {
            double prcMass = Double.parseDouble(compoundElement.getChildTextTrim("prc_mass"));
            char symbol = compoundElement.getChildText("symbol").charAt(0);
            Element fragBranch = compoundElement.getChild("frag");
            String bIonListStr = fragBranch.getChildTextTrim("b");
            String yIonListStr = fragBranch.getChildTextTrim("y");
            List<Double> bIonList = new ArrayList<>();
            List<Double> yIonList = new ArrayList<>();

            String [] bArr = bIonListStr.split(";");
            for(String b: bArr)
            {
                bIonList.add(Double.parseDouble(b));
            }
            String [] yArr = yIonListStr.split(";");
            for(String y: yArr)
            {
                yIonList.add(Double.parseDouble(y));
            }
            CompoundDiffMod compoundDiffMod = new CompoundDiffMod(prcMass,symbol, bIonList, yIonList);
            compoundDiffMod.setModInfo("("+prcMass+")");
            compoundDiffMod.setModifiable((byte)symbol, true);
            diffMods.add(compoundDiffMod);


            //System.out.println(">>>"+prcMass+"\t"+bIonList+"\t"+yIonList);
        }
