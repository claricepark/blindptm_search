...
/**
 * Created by yateslab on 3/28/17.
 */
public class DistributionSmallCalculator {
    ...
    // for ptrue > 0.1
    public static int[][][] binomial = new int[100][N][];
    // will be used for 0.001 < ptrue < 0.1
    public static int[][][] binomial2 = new int[100][N][];
    private static boolean isLoaded = false;
    public DistributionSmallCalculator() {
        if(!isLoaded) {
            load();
            isLoaded = true;
...
//System.out.println("percentage: " + percentage + "\tn: " + n + "\tk: " + k);
            percentage = percentage > 1? percentage : 1;
            return binomial[percentage][n][k] / Math.pow(100,k);
        } catch (ArrayIndexOutOfBoundsException e) {
            String message;
            if(percentage < 0 || percentage > 100) {
                message = "Percentage should be between 0 and 100";
    ...
    public static void load() {
        ...
        for(int i = 1; i < 100; i++) {
            for(int j = 1; j < N; j++) {
                int size = j + 1;
                binomial[i][j] = new int[size];
                binomial2[i][j] = new int[size];
                for(int k = 0; k < size; k++) {
                    ...
                    */
                    binomial[i][j][k] = calcBinomialProbability(i, j, k);
                    binomial2[i][j][k] = calcBinomialProbability(i/10, j, k);
    ...
    public static int calcBinomialProbability(int p, int n, int k) {
        int result = 1;
        int q = 100 - p;
        int nMinusK = n - k;
/*
...
//System.out.println("\t" + result);
    ...
    public static void main(String [] args) {
        ...
        DistributionSmallCalculator dc = new DistributionSmallCalculator();
        ///BinomialDistribution binomialDistribution = new BinomialDistribution(30,0.3);
        //System.out.println(binomialDistribution.probability(20));
        System.out.println(DistributionCalculator.calcBinomialSum(0.3,30,20));
