...
//    private int totalPeptideLength = 0;
    ...
    private Zline zline;
    private boolean printOnlySLine = false;
    private boolean notSearched = false;
    private PeakList peakList;
//    private static DistributionCalculator dc = new DistributionCalculator();
    public SearchResult()
    {
        minNumPeaksMatched =-1;
        minPeptideLength = -1;
        printOnlySLine = true;
        notSearched = true;
    }
    public SearchResult(Zline zline, PeakList peakList) {
        this.zline = zline;
        minNumPeaksMatched =-1;
        minPeptideLength = -1;
        printOnlySLine = true;
...
//System.out.println(NUMFINALRESULT + "\t" + i + "\t" + (i < NUMFINALRESULT) + "\t" +  !topseq.equals(sph.getOriginalSequence()));
    ...
    public String outputResults() {
        ...
        appendSline(result);
        if(!printOnlySLine)appendMlines(result); // L lines are also appended
        result.append('\n');
/*
...
*/
    ...
    private void appendMline(StringBuffer result, ScoredPeptideHit sph) {
        ...
        result.append(DELIMITER);
        //result.append(threeDigits.format(sph.getSecondaryScore())); // XCorr
   /*     result.append(sph.getPScore()); // for probablity scores, cannot use format
        result.append(DELIMITER);
        result.append(sph.getPValue()); // for probablity scores, cannot use format
        result.append(DELIMITER);
        result.append(sph.getEValue()); // for probablity scores, cannot use format
        result.append(DELIMITER);*/
        ...
        for(PeptideHit p : sph.getPeptideHits()) {
            ...
            result.append("\t" + p.getStart() + "\t" + p.getExtraExtendedSequence() + "\n");
        }
    }
       ...
       if(notSearched)
        {
            result.append("S");
            result.append(DELIMITER);
            result.append(peakList.getHiscan());
            result.append(DELIMITER);
            result.append(peakList.getLoscan());
            result.append(DELIMITER);
            ...
            result.append(DELIMITER);
            result.append(fiveDigits.format(zline.getM2z()));
            result.append(DELIMITER);
            result.append(twoDigits.format(peakList.getTotalIntensity()));
            result.append(DELIMITER);
            result.append("NA"); // for ptrue
            result.append(DELIMITER);
...
//       System.out.println("Number of Elements for zscore: " + numElement);        
    ...
    public void addMPeptideHit(PeptideHit p) {
        //if(isValidHit(p)) {
        addFinalPeptideHit(p);
        //}
    }
    ...
    /*    if(p.getSequence().equals("LSMEEPTIRTRNSDEID"))
        {
            System.out.println("<<<>> "+p.getExtendedSequence() +"\t"+p.getNumPeaksMatched()+"\t"+p.getNumPeaks());
        }*/
...
//System.out.println(seq + "\t" + hits.size());
    ...
    public boolean getPrintOnlySLine()
    {
        return printOnlySLine;
    }
    public void setPrintOnlySLine(boolean b)
    {
        printOnlySLine = b;
    }
