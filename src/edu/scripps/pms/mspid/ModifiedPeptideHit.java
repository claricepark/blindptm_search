...
public class ModifiedPeptideHit extends PeptideHit {
    ...
    private Map<Integer,List<String>> polyMorphMap  = null;
    private IndexedSequenceWithPTMs iseq  = null;
    ...
    public boolean isGoodCandidate() {
        int realNumInternalDiffModsAdded = numInternalDiffModsAdded;
        realNumInternalDiffModsAdded = mods.getCTerm()!=null ? realNumInternalDiffModsAdded +1 : realNumInternalDiffModsAdded;
        realNumInternalDiffModsAdded = mods.getNTerm()!=null ? realNumInternalDiffModsAdded +1 : realNumInternalDiffModsAdded;

        return Math.abs(internalDiffModMassAdded - mods.getInternalDiffModsMass()) <0.001 &&
                realNumInternalDiffModsAdded == mods.getNumDiffMods();
    }
...
//System.out.print("numPotentialSitesInPeptide: " + numPotentialSites +  "\t numDiffMods: " + mods.getNumDiffMods());
    ...
    /*    if(mods.getNumDiffMods()>1)
        {
            for(ModifiedPeptideHit peptideHit: candidates)
            {
                System.out.println("<<>> "+peptideHit.getExtendedSequence());
            }
        }*/
    ...
    private ArrayList<ModifiedPeptideHit> getFinalCandidates(ArrayList<ModifiedPeptideHit> candidates) {
        ...
        if(temp.get(0).getSequence().equals("GYQETDSVVSSVTTKAKGV"))
        {
...
//System.out.println("numDiffModsForThisSite: " + dms.numDiffMods());
        ...
        for(int i = 0; i < num; i++) {
            ...
            for(Iterator<DiffMod> it = dms.getDiffMods(); it.hasNext();) {
                ...
                if(m.numInternalDiffModsAdded+1 <= mods.getNumDiffMods() &&
                          m.internalDiffModMassAdded+d.getMassShift()-0.001 <= mods.getInternalDiffModsMass()) {
    ...
    protected void calcModificationSites() {
        ...
        numPotentialSites = 0;
        if(mods.getCTerm()!=null)
        {
            numPotentialSites++;
        }
        if(mods.getNTerm()!=null)
        {
            numPotentialSites++;
        }
        int start = getStart();
        ...
        for(int i = start; i <= end; i++) {
            if(mods.isModifiable(bytes[i])) {
                boolean cTermCheck = true;
              /*  if(i ==end && mods.getCTerm()!=null )
                {
                    cTermCheck = false;
                }*/
                if(cTermCheck)
...
//System.out.println("in getCTermStart in ModifiedPeptideHit, " + mods.getDiffCTermMod());
    ...
    public String getExactSequence() {
       ...
       // sb.append(getSequence()); // need to be modified for regular mod search
        for(int i = start; i <= end; i++) {
            if(polyMorphMap!=null)
            {
                int index = i -start;
                if(polyMorphMap.containsKey(index))
                {
                    List<String> polyList =  polyMorphMap.get(index) ;
...
*/
    ...
    public void getNLChargeNNumPeaksMatched(int z) {
        ...
        for(int i = start; i < end; i++) {
           ...
           /* if(polyMorphMap!=null)
            {
                y = polyMorphMap.containsKey(ymi) ? polyMorphMap.get(ymi) : y;
                b = polyMorphMap.containsKey(bmi) ? polyMorphMap.get(bmi) : b;
            }*/
            ...
            bMass += mc.getFragmentMass(b);
            yMass += mc.getFragmentMass(y);
...
*/
            ...
            DiffMod bd = diffMods[bmi];
            if( bd instanceof  CompoundDiffMod)
            {
                CompoundDiffMod cbd = (CompoundDiffMod) bd;
                for(double d: cbd.getbIonList())
                {
                    double tempBmass = bMass +d;
                    int int2B = (int)(tempBmass*ppl.PRECISIONFACTOR + 0.5f);
                    int int3B = (int) (0.5f + ppl.PRECISIONFACTOR*(tempBmass + MassSpecConstants.MASSPROTON)/2f);
                    if(int3B < lastTrue && int3B > firstTrue) {
                        numTheroticPeaks++;
                        if(boolMasses[int3B]) numPeaksMatched++;
                    }
                    ...
                    for(int j = 3; j <= z; j++) {
                        int nb = (int)(0.5f + ppl.PRECISIONFACTOR*(tempBmass + MassSpecConstants.MASSPROTON)/j);
                        if(nb < lastTrue && nb > firstTrue) {
                            numTheroticPeaks++;
                            if(boolMasses[nb]) numPeaksMatched++;
                        }
                ...
                bMass += cbd.getbIonList().get(cbd.getbIonList().size()-1);
            }
            else
            {
                bMass += bd == null? 0 : bd.getMassShift();
            ...
            DiffMod yd = diffMods[ymi];
            if(yd instanceof  CompoundDiffMod)
            {
                CompoundDiffMod cyd = (CompoundDiffMod) yd;
                for(double d: cyd.getyIonlist())
                {
                    double tempYMass = yMass +d;
                    int int2Y = (int)(tempYMass*ppl.PRECISIONFACTOR + 0.5f);
                    int int3Y = (int)(0.5f + ppl.PRECISIONFACTOR*(tempYMass + MassSpecConstants.MASSPROTON)/2f);
                    ...
                    for(int j = 3; j <= z; j++) {
                        int ny = (int)(0.5f + ppl.PRECISIONFACTOR*(tempYMass + MassSpecConstants.MASSPROTON)/j);
                        if(ny < lastTrue && ny > firstTrue) {
                            numTheroticPeaks++;
                            if(boolMasses[ny]) numPeaksMatched++;
                        }
                ...
                yMass += cyd.getyIonlist().get(cyd.getyIonlist().size()-1);
            }
            else
            {
                yMass += yd == null? 0 : yd.getMassShift();
...
*/
    ...
    public void getCharge2NumPeaksMatched() {
        ...
        int firstTrue = ppl.getFirstTrue() - 1;
        int lastTrue = ppl.getLastTrue() + 1;
     /*   if(this.getExtendedSequence().equals("K.LS(79.9663)MEEPT(79.9663)IRTRNS(79.9663)DEID.I")) {
            System.out.println("<<>><< ");
        }*/
...
*/
            ...
            if( bd instanceof  CompoundDiffMod)
            {
                ...
                {
                    double tempBMass = bMass +d;
                    int tempB = (int)(tempBMass * ppl.PRECISIONFACTOR+ 0.5f);
                    if(tempB < lastTrue && tempB > firstTrue) {
                        numTheroticPeaks++;
                        if(boolMasses[tempB]) numPeaksMatched++;
                    }
             ...
             /*   if(this.getExtendedSequence().equals("K.LS(79.9663)MEEPT(79.9663)IRTRNS(79.9663)DEID.I"))
                ...
                if(tempB < lastTrue && tempB > firstTrue) {
                 ...
                 /*   if(this.getExtendedSequence().equals("K.LS(79.9663)MEEPT(79.9663)IRTRNS(79.9663)DEID.I"))
                    ...
                    if(boolMasses[tempB])
                    {
                        /*if(this.getSequence().equals("LSMEEPTIRTRNSDEID"))
                        {
                            System.out.println("<<<>> "+tempB);
                        }*/
                        numPeaksMatched++;
            ...
            if( yd instanceof  CompoundDiffMod)
            {
                ...
                {
                    double tempYMass = yMass + d;
                    int tempY = (int)(tempYMass*ppl.PRECISIONFACTOR + 0.5f);
                    if(tempY < lastTrue && tempY > firstTrue) {
                        numTheroticPeaks++;
                        if(boolMasses[tempY]) numPeaksMatched++;
                    }
           ...
           /* if(this.getExtendedSequence().equals("K.LS(79.9663)MEEPT(79.9663)IRTRNS(79.9663)DEID.I")) {
                System.out.println("<<>><< " + bMass + "\t" + yMass);
            }
*/
    ...
    public int [] getNLCharge2TheorMasses() {
        ...
        if(!SearchParams.isHighResolution)
        {
            bMass+=0.5;
            yMass+=0.5;
        }
...
/*
            if(polyMorphMap!=null)
            {
                y = polyMorphMap.containsKey(yi) ? polyMorphMap.get(yi) : y;
                b = polyMorphMap.containsKey(i) ? polyMorphMap.get(i) : b;
            }
*/
            ...
            if(bm !=null && bm instanceof CompoundDiffMod)
            {
                CompoundDiffMod cbm = (CompoundDiffMod) bm;
                for(double d: cbm.getbIonList())
                {
                    double tempBMass = bMass +d*MassSpecConstants.DBINWIDTH;
                    processSinglyChargedBIon(theorMass, tempBMass);
                    if(bneutralloss == 0 && bm != null) bneutralloss = bm.getDbinwidthNeutralLoss();
                    if(bneutralloss != 0) processSinglyChargedBIon(theorMass, tempBMass - bneutralloss);
                }
                bMass+=cbm.getbIonList().get(cbm.getbIonList().size()-1);
            }
            else
            {
            ...
            if(ym!=null && ym instanceof  CompoundDiffMod)
            {
                CompoundDiffMod cym = (CompoundDiffMod) ym;
                for(double d: cym.getyIonlist())
                {
                    double tempYMass = yMass+d*MassSpecConstants.DBINWIDTH;
                    processSinglyChargedYIon(theorMass, tempYMass);
                    if(yneutralloss == 0 && ym != null) yneutralloss = ym.getDbinwidthNeutralLoss();
                    if(yneutralloss != 0) processSinglyChargedYIon(theorMass, tempYMass - yneutralloss);
                }
                yMass+=cym.getyIonlist().get(cym.getyIonlist().size()-1);
            }
            else
            {
...
*/
            ...
            // specific for diff mod
            DiffMod bm = diffMods[i];
            if(bm instanceof  CompoundDiffMod)
            {
                CompoundDiffMod cbm = (CompoundDiffMod)bm;
                for(double d: cbm.getbIonList())
                {
                    double tempBMass = bMass + d*MassSpecConstants.DBINWIDTH;
                    if(bneutralloss == 0 && bm != null) bneutralloss = bm.getDbinwidthNeutralLoss();
                    processSinglyChargedBIon(theorMass, tempBMass - bneutralloss);
                }
                bMass += cbm.getbIonList().get(cbm.getbIonList().size()-1);
            }
            else
            ...
            DiffMod ym = diffMods[yi];
            if(ym instanceof CompoundDiffMod)
            {
                CompoundDiffMod cym = (CompoundDiffMod)ym;
                for(double d: cym.getyIonlist())
                {
                    double tempYMass = yMass + d*MassSpecConstants.DBINWIDTH;
                    if(yneutralloss == 0 && ym != null) yneutralloss = ym.getDbinwidthNeutralLoss();
                    processSinglyChargedYIon(theorMass, tempYMass - yneutralloss);
                }
            }
            else
            {
...
*/
            ...
            if( bm instanceof CompoundDiffMod)
            {
                ...
                double tempBMass;
                for(double d: cbm.getbIonList()) {
                     tempBMass = bMass + d * MassSpecConstants.DBINWIDTH;
                    processSinglyChargedBIon(theorMass, tempBMass);
                }
                bMass += cbm.getbIonList().get(cbm.getbIonList().size()-1);
            }
...
*/
            ...
            if(bm !=null && bm instanceof  CompoundDiffMod)
            {
                ...
                for(double d: cbm.getbIonList())
                {
                    double tempBMass = bMass + d * MassSpecConstants.DBINWIDTH;
                    processSinglyChargedBIon(theorMass, tempBMass);
                    processDoublyChargedBIon(theorMass, tempBMass);
                }
                bMass += cbm.getbIonList().get(cbm.getbIonList().size()-1);
            }
            else
            ...
            if(ym != null && ym instanceof  CompoundDiffMod)
            {
                ...
                for( double d: cym.getyIonlist())
                {
                    double tempYMass = yMass + d * MassSpecConstants.DBINWIDTH;
                    processSinglyChargedYIon(theorMass, tempYMass);
                    processDoublyChargedYIon(theorMass, tempYMass);
                }
                yMass += cym.getyIonlist().get(cym.getyIonlist().size()-1);
            }
            else
...
*/
            ...
            if(bm!=null && bm instanceof  CompoundDiffMod)
            {
                ...
                for(double d: cbm.getbIonList())
                {
                    ...
                    processSinglyChargedBIon(theorMass, tempBMass);
                    processDoublyChargedBIon(theorMass, tempBMass);
                    processHighlyChargedIons(theorMass,tempBMass,z);
                }
                bMass += cbm.getbIonList().get(cbm.getbIonList().size()-1);
            }
            else
            {
                ...
                processSinglyChargedBIon(theorMass, bMass);
                processDoublyChargedBIon(theorMass, bMass);
                processHighlyChargedIons(theorMass,bMass,z);
            ...
            if(ym != null && ym instanceof  CompoundDiffMod)
            {
                ...
                for( double d: cym.getyIonlist())
                {
                    ...
                    processSinglyChargedYIon(theorMass, tempYMass);
                    processDoublyChargedYIon(theorMass, tempYMass);
                    processHighlyChargedIons(theorMass,tempYMass,z);
                }
                yMass += cym.getyIonlist().get(cym.getyIonlist().size()-1);
            }
            else
            {
                ...
                processSinglyChargedYIon(theorMass, yMass);
                processDoublyChargedYIon(theorMass, yMass);
                processHighlyChargedIons(theorMass,yMass,z);
            }
            // for highly charged fragment ions
            //processHighlyChargedIons(theorMass, bMass, yMass, z);
        }
    ...
    public Map<Integer,List<String>> getPolyMorphMap() {
        return polyMorphMap;
    }

    public void setPolyMorphMap(Map<Integer,List<String>> polyMorphMap) {
        this.polyMorphMap = polyMorphMap;
    }

    public IndexedSequenceWithPTMs getIseq() {
        return iseq;
    }

    public void setIseq(IndexedSequenceWithPTMs iseq) {
        this.iseq = iseq;
    }
}
